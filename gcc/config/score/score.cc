...
{
  score_address_type type;
  rtx reg;
  rtx offset;
  rtx_code code;
  score_symbol_type symbol_type;
};
#endif
...
HOST_WIDE_INT
score_constant_alignment (const_tree exp, HOST_WIDE_INT basic_align)
{
  return (TREE_CODE (exp) == STRING_CST  || TREE_CODE (exp) == CONSTRUCTOR)  
   && (basic_align) < BITS_PER_WORD ? BITS_PER_WORD : (basic_align);
}

unsigned int
score_hard_regno_nregs (unsigned regno, machine_mode mode)
{
...
score_subw (rtx op, int high_p)
{
  ...
  unsigned int byte = (TARGET_LITTLE_ENDIAN ? high_p : !high_p) ? UNITS_PER_WORD : 0;
...
static score_frame_info *
score_cached_frame (void)
{
  static score_frame_info _frame_info;
  return &_frame_info;
}
...
{
  score_frame_info *f = score_cached_frame ();
  *f = {};
  f->gp_reg_size = 0;
  f->mask = 0;
...
score_classify_address (struct score_address_info *info,
                        machine_mode mode, rtx x, int strict)
{
                       ...
                       tree function)
{
  ...
  rtx insn = emit_call_insn (gen_sibcall_internal_score7 (fnaddr, const0_rtx));
  ...
  shorten_branches (static_cast<rtx_insn*>(insn));
  final_start_function (static_cast<rtx_insn*>(insn), file, 1);
  final (static_cast<rtx_insn*>(insn), file, 1);
  final_end_function ();
...
static void
score_classify_arg (cumulative_args_t cum_args,
                    const function_arg_info &arg, machine_mode mode,
                    const_tree type, bool named, struct score_arg_info *info)
{
  unsigned int num_words, max_regs;
  CUMULATIVE_ARGS *cum = get_cumulative_args (cum_args);

  bool even_reg_p = false;
...
/* Choose the section to use for the constant rtx expression X that has
   mode MODE.  */
static section *
score_select_rtx_section (machine_mode mode, rtx x, unsigned HOST_WIDE_INT align)
{
...
score_asm_file_end (void)
{
  if (extern_head)
    {
      fputs ("\n", asm_out_file);
      for (auto *p = extern_head; p != 0; p = p->next)
        {
...
reg_class
score_preferred_reload_class (rtx x ATTRIBUTE_UNUSED, reg_class rclass)
{
...
reg_class
score_secondary_reload_class (reg_class rclass,
                              machine_mode,
                              rtx x)
{
  int regno = -1;
...
bool
score_hard_regno_mode_ok (unsigned regno, machine_mode mode)
{
...
{
  score_frame_info *f = score_compute_frame_size (get_frame_size ());
  switch (from)
    {
...
{
  score_arg_info info;
  score_classify_arg (cum_args, arg, arg.mode, arg.type, arg.named, &info);
  CUMULATIVE_ARGS *cum = get_cumulative_args (cum_args);
...
score_function_arg (cumulative_args_t cum_args, const function_arg_info &arg)
{
  ...
  if (!info.stack_words)
  ...
  else
    {
      ...
      for (unsigned int i = 0; i < info.reg_words; i++)
        {
          rtx reg;
...
rtx
score_function_value (const_tree valtype, const_tree func, machine_mode mode)
{
  if (valtype)
    {
...
#define CODE_SIZE        (TRAMPOLINE_INSNS * UNITS_PER_WORD)
  ...
  rtx mem = adjust_address (m_tramp, SImode, CODE_SIZE);
...
static bool
score_legitimate_address_p (machine_mode mode, rtx x, bool strict)
{
...
/* Implement TARGET_REGISTER_MOVE_COST.
   ...
   FROM to class TO. */
static int
score_register_move_cost (machine_mode,
...
static int
score_symbol_insns (score_symbol_type type)
{
  switch (type)
    {
    case SYMBOL_GENERAL:
...
static int
score_address_insns (rtx x, machine_mode mode)
{
  int factor;
...
bool
score_rtx_costs (rtx x, machine_mode, int outer_code, int opno, int *total, bool)
{
...
int
score_address_cost (rtx addr, machine_mode, addr_space_t, bool)
{
                       ...
                       tree decl, const char *name)
{
  if (score_in_small_data_p (decl))
    {
      extern_list *p = ggc_alloc<extern_list> ();
      p->next = extern_head;
      p->name = name;
...
machine_mode
score_select_cc_mode (rtx_code op, rtx x, rtx y)
{
...
#define EMIT_PL(_rtx)        RTX_FRAME_RELATED_P (_rtx) = 1
  ...
  HOST_WIDE_INT size = f->total_size - f->gp_reg_size;
  if (flag_pic)
...
score_epilogue (int sibcall_p)
{
  score_frame_info *f = score_compute_frame_size (get_frame_size ());
  HOST_WIDE_INT size = f->total_size - f->gp_reg_size;
  rtx base = !frame_pointer_needed ? stack_pointer_rtx : hard_frame_pointer_rtx;
  if (size)
    {
      if (size >= -32768 && size <= 32767)
  ...
  for (int regno = (int) GP_REG_FIRST; regno <= (int) GP_REG_LAST; regno++)
    {
      if (BITSET_P (f->mask, regno - GP_REG_FIRST))
        {
          rtx mem = gen_rtx_MEM (SImode,
...
int
score_symbolic_constant_p (rtx x, score_symbol_type *symbol_type)
{
  HOST_WIDE_INT offset;
...
{
  machine_mode mode = score_select_cc_mode (GET_CODE (ops[1]), ops[2], ops[3]);
...
score_zero_extract_andi (rtx *ops)
{
  if (INTVAL (ops[1]) == 1 && const_uimm5 (ops[2], SImode))
    ...
    {
      unsigned HOST_WIDE_INT mask = (0xffffffffU & ((1U << INTVAL (ops[1])) - 1U));
...
static int
score_pr_addr_post (rtx *ops, int idata, int iaddr, char *ip, score_mem_unit unit)
{
  score_address_info ai;
...
const char *
score_linsn (rtx *ops, score_mem_unit unit, bool sign)
{
  const char *pre_ins[] =
    {"lbu", "lhu", "lw", "??", "lb", "lh", "lw", "??"};
  strcpy (score_ins, pre_ins[(sign ? 4 : 0) + unit]);
  char *ip = score_ins + strlen (score_ins);
...
const char *
score_sinsn (rtx *ops, score_mem_unit unit)
{
...
static bool
score_function_ok_for_sibcall (tree, tree)
{
  return true;
}
...
void
score_init_cumulative_args (CUMULATIVE_ARGS *cum, tree, rtx)
{
  *cum = {};
}
