...
static const struct processor_costs z900_cost =
    {
        COSTS_N_INSNS(5),   /* M     */
...
static const struct processor_costs z990_cost =
    {
        COSTS_N_INSNS(4),   /* M     */
...
static const struct processor_costs z9_109_cost =
    {
        COSTS_N_INSNS(4),   /* M     */
        COSTS_N_INSNS(2),   /* MGHI  */
        COSTS_N_INSNS(2),   /* MH    */
...
static const struct processor_costs z10_cost =
    {
        COSTS_N_INSNS(10),  /* M     */
        COSTS_N_INSNS(10),  /* MGHI  */
        COSTS_N_INSNS(10),  /* MH    */
...
static const struct processor_costs z196_cost =
    {
        COSTS_N_INSNS(7),   /* M     */
...
static const struct processor_costs zEC12_cost =
    {
        COSTS_N_INSNS(7),   /* M     */
        COSTS_N_INSNS(5),   /* MGHI  */
        COSTS_N_INSNS(5),   /* MH    */
...
bool s390_return_addr_from_memory()
{
  return cfun_gpr_save_slot(RETURN_REGNUM) == SAVE_SLOT_STACK;
}

/* Return nonzero if it's OK to use fused multiply-add for MODE.  */
bool s390_fma_allowed_p(machine_mode mode)
{
...
#define DEF_TYPE(INDEX, NODE, CONST_P)   \
  if (s390_builtin_types[INDEX] == NULL) \
    s390_builtin_types[INDEX] = (!CONST_P) ? (NODE) : build_type_variant((NODE), 1, 0);
...
#define OB_DEF(NAME, FIRST_VAR_NAME, LAST_VAR_NAME, BFLAGS, FNTYPE)                  \
  if (s390_builtin_decls[S390_OVERLOADED_BUILTIN_##NAME + S390_BUILTIN_MAX] == NULL) \
...
/* Return true if ARG is appropriate as argument number ARGNUM of
   ...
   passed as OP_FLAGS.  */
bool s390_const_operand_ok(tree arg, int argnum, int op_flags, tree decl)
{
  if (O_UIMM_P(op_flags))
  {
    ...
    if (!tree_fits_uhwi_p(arg) || tree_to_uhwi(arg) > (HOST_WIDE_INT_1U << bitwidth) - 1 || (bitmask && tree_to_uhwi(arg) & ~bitmask))
    {
      if (bitmask)
      {
  ...
  if (O_SIMM_P(op_flags))
  {
    ...
    if (!tree_fits_shwi_p(arg) || tree_to_shwi(arg) < -(HOST_WIDE_INT_1 << (bitwidth - 1)) || tree_to_shwi(arg) > ((HOST_WIDE_INT_1 << (bitwidth - 1)) - 1))
    {
...
#define MAX_ARGS 6
  ...
  if (S390_USE_TARGET_ATTRIBUTE)
  {
    ...
    if (((bflags & B_VX) || (bflags & B_VXE)) && !TARGET_VX)
    {
      error("builtin %qF requires %<-mvx%> "
            "(default with %<-march=z13%> and higher)",
            fndecl);
      return const0_rtx;
    }
  ...
  if (fcode >= S390_OVERLOADED_BUILTIN_VAR_OFFSET && fcode < S390_ALL_BUILTIN_MAX)
  {
    gcc_unreachable();
  }
 ...
 saving/restoring of FPRs.  */
  ...
  if (nonvoid)
  {
    ...
    if (!target || GET_MODE(target) != tmode || !(*insn_data[icode].operand[0].predicate)(target, tmode))
 ...
 operation.  */
  ...
  FOR_EACH_CALL_EXPR_ARG(arg, iter, exp)
  {
    ...
    if (O_IMM_P(op_flags) && TREE_CODE(arg) != INTEGER_CST)
    {
 ...
 being done.  */
    if (CONST_INT_P(op[arity]) && SCALAR_INT_MODE_P(insn_op->mode) && GET_MODE(op[arity]) != insn_op->mode)
 ...
 patterns not just accepting MEMs.  */
    if (POINTER_TYPE_P(TREE_TYPE(arg)) && insn_op->predicate != address_operand)
 ...
 Try making the address legal by copying it into a register.  */
    if (MEM_P(op[arity]) && insn_op->predicate == memory_operand && (GET_MODE(XEXP(op[arity], 0)) == Pmode || GET_MODE(XEXP(op[arity], 0)) == VOIDmode))
    {
 ...
 arguments is mapped to addti3.  */
    else if (insn_op->mode != VOIDmode && GET_MODE(op[arity]) != VOIDmode && GET_MODE(op[arity]) != insn_op->mode && ((tmp_rtx = simplify_gen_subreg(insn_op->mode, op[arity], GET_MODE(op[arity]), 0)) != NULL_RTX))
    {
      op[arity] = tmp_rtx;
    }
 ...
 Copy the operand to register.  */
    if (!insn_op->predicate(op[arity], insn_op->mode) && (GET_MODE(op[arity]) == insn_op->mode || GET_MODE(op[arity]) == VOIDmode || (insn_op->predicate == address_operand && GET_MODE(op[arity]) == Pmode)))
    {
                               ...
                               int flags ATTRIBUTE_UNUSED, bool *no_add_attrs)
{
  ...
  if (args == NULL || TREE_CHAIN(args) == NULL)
    err = 1;
  else if (TREE_CODE(expr) != INTEGER_CST || !INTEGRAL_TYPE_P(TREE_TYPE(expr)) || wi::gtu_p(wi::to_wide(expr), s390_hotpatch_hw_max))
    err = 1;
  else if (TREE_CODE(expr2) != INTEGER_CST || !INTEGRAL_TYPE_P(TREE_TYPE(expr2)) || wi::gtu_p(wi::to_wide(expr2), s390_hotpatch_hw_max))
    err = 1;
  else
    err = 0;
  if (err)
                                 ...
                                 int flags ATTRIBUTE_UNUSED, bool *no_add_attrs)
{
  ...
  while (POINTER_TYPE_P(type) || TREE_CODE(type) == FUNCTION_TYPE || TREE_CODE(type) == METHOD_TYPE || TREE_CODE(type) == ARRAY_TYPE)
    type = TREE_TYPE(type);
...
/* Expand the naked attribute.  */

static tree
s390_handle_naked_attribute(tree *node, tree name ATTRIBUTE_UNUSED,
                            tree args ATTRIBUTE_UNUSED,
                            int flags ATTRIBUTE_UNUSED, bool *no_add_attrs)
{
                             ...
                             bool *no_add_attrs)
{
  ...
  if (is_attribute_p("indirect_branch", name) || is_attribute_p("indirect_branch_call", name) || is_attribute_p("function_return", name) || is_attribute_p("function_return_reg", name) || is_attribute_p("function_return_mem", name))
  {
    if (strcmp(TREE_STRING_POINTER(cst), "keep") != 0 && strcmp(TREE_STRING_POINTER(cst), "thunk") != 0 && strcmp(TREE_STRING_POINTER(cst), "thunk-extern") != 0)
    {
      warning(OPT_Wattributes,
  ...
  if (is_attribute_p("indirect_branch_jump", name) && strcmp(TREE_STRING_POINTER(cst), "keep") != 0 && strcmp(TREE_STRING_POINTER(cst), "thunk") != 0 && strcmp(TREE_STRING_POINTER(cst), "thunk-inline") != 0 && strcmp(TREE_STRING_POINTER(cst), "thunk-extern") != 0)
  {
    warning(OPT_Wattributes,
            "argument to %qE attribute is not "
...
static const struct attribute_spec s390_attribute_table[] = {
    ...
    {"naked", 0, 0, true, false, false, false,
     s390_handle_naked_attribute, NULL},
...
int s390_label_align(rtx_insn *label)
{
  ...
  if (GET_CODE(src) == UNSPEC && XINT(src, 1) == UNSPEC_MAIN_BASE)
    return 0;
...
s390_vector_mode_supported_p(machine_mode mode)
{
  ...
  if (!VECTOR_MODE_P(mode) || !TARGET_VX || GET_MODE_SIZE(mode) > 16)
    return false;
...
void s390_set_has_landing_pad_p(bool value)
{
...
s390_cc_modes_compatible(machine_mode m1, machine_mode m2)
{
  ...
  {
  case E_CCZmode:
    if (m2 == CCUmode || m2 == CCTmode || m2 == CCZ1mode || m2 == CCSmode || m2 == CCSRmode || m2 == CCURmode)
      return m2;
    return VOIDmode;
...
s390_match_ccmode_set(rtx set, machine_mode req_mode)
{
  ...
  /* These modes are supposed to be used only in CC consumer
     patterns.  */
  gcc_assert(req_mode != CCVIALLmode && req_mode != CCVIANYmode && req_mode != CCVFALLmode && req_mode != CCVFANYmode);
  ...
  case E_CCZmode:
    if (req_mode != CCSmode && req_mode != CCUmode && req_mode != CCTmode && req_mode != CCSRmode && req_mode != CCURmode && req_mode != CCZ1mode)
      return 0;
    break;
...
bool s390_match_ccmode(rtx_insn *insn, machine_mode req_mode)
{
  int i;
...
s390_select_ccmode(enum rtx_code code, rtx op0, rtx op1)
{
  ...
  case EQ:
  case NE:
    if ((GET_CODE(op0) == NEG || GET_CODE(op0) == ABS) && GET_MODE_CLASS(GET_MODE(op0)) == MODE_INT)
      return CCAPmode;
    if (GET_CODE(op0) == PLUS && GET_CODE(XEXP(op0, 1)) == CONST_INT && CONST_OK_FOR_K(INTVAL(XEXP(op0, 1))))
      return CCAPmode;
    if ((GET_CODE(op0) == PLUS || GET_CODE(op0) == MINUS || GET_CODE(op1) == NEG) && GET_MODE_CLASS(GET_MODE(op0)) == MODE_INT)
      return CCLmode;
    ...
    if (register_operand(op0, HImode) && GET_CODE(op1) == CONST_INT && (INTVAL(op1) == -1 || INTVAL(op1) == 65535))
      return CCT3mode;
    if (register_operand(op0, QImode) && GET_CODE(op1) == CONST_INT && (INTVAL(op1) == -1 || INTVAL(op1) == 255))
      return CCT3mode;
  ...
  case GT:
    ...
    if (GET_CODE(op0) == PLUS && GET_CODE(XEXP(op0, 1)) == CONST_INT && (CONST_OK_FOR_K(INTVAL(XEXP(op0, 1))) || (CONST_OK_FOR_CONSTRAINT_P(INTVAL(XEXP(op0, 1)), 'O', "Os")
  ...
  case UNGE:
  case UNGT:
    if ((GET_CODE(op0) == SIGN_EXTEND || GET_CODE(op0) == ZERO_EXTEND) && GET_CODE(op1) != CONST_INT)
      return CCSRmode;
    return CCSmode;

  case LTU:
  case GEU:
    if (GET_CODE(op0) == PLUS && GET_MODE_CLASS(GET_MODE(op0)) == MODE_INT)
      return CCL1mode;
  ...
  case LEU:
  case GTU:
    if (GET_CODE(op0) == MINUS && GET_MODE_CLASS(GET_MODE(op0)) == MODE_INT)
      return CCL2mode;
                             ...
                             bool op0_preserve_value)
{
  ...
  if ((*code == EQ || *code == NE) && *op1 == const0_rtx && GET_CODE(*op0) == ZERO_EXTRACT && GET_CODE(XEXP(*op0, 1)) == CONST_INT && GET_CODE(XEXP(*op0, 2)) == CONST_INT && SCALAR_INT_MODE_P(GET_MODE(XEXP(*op0, 0))))
  {
    ...
    if (len > 0 && len < modesize && pos >= 0 && pos + len <= modesize && modesize <= HOST_BITS_PER_WIDE_INT)
    {
  ...
  if ((*code == EQ || *code == NE) && *op1 == const0_rtx && GET_CODE(*op0) == AND && GET_CODE(XEXP(*op0, 1)) == CONST_INT && SCALAR_INT_MODE_P(GET_MODE(XEXP(*op0, 0))))
  {
    rtx inner = XEXP(*op0, 0);
    rtx mask = XEXP(*op0, 1);

    /* Ignore paradoxical SUBREGs if all extra bits are masked out.  */
    if (GET_CODE(inner) == SUBREG && SCALAR_INT_MODE_P(GET_MODE(SUBREG_REG(inner))) && (GET_MODE_SIZE(GET_MODE(inner)) >= GET_MODE_SIZE(GET_MODE(SUBREG_REG(inner)))) && ((INTVAL(mask) & GET_MODE_MASK(GET_MODE(inner)) & ~GET_MODE_MASK(GET_MODE(SUBREG_REG(inner)))) == 0))
  ...
  if ((*code == EQ || *code == NE) && GET_CODE(*op1) == CONST_INT && INTVAL(*op1) == 0xffff && SCALAR_INT_MODE_P(GET_MODE(*op0)) && (nonzero_bits(*op0, GET_MODE(*op0)) & ~HOST_WIDE_INT_UC(0xffff)) == 0)
  {
  ...
  if (GET_CODE(*op0) == UNSPEC && XINT(*op0, 1) == UNSPEC_STRCMPCC_TO_INT && XVECLEN(*op0, 0) == 1 && GET_MODE(XVECEXP(*op0, 0, 0)) == CCUmode && GET_CODE(XVECEXP(*op0, 0, 0)) == REG && REGNO(XVECEXP(*op0, 0, 0)) == CC_REGNUM && *op1 == const0_rtx)
  {
  ...
  if (GET_CODE(*op0) == UNSPEC && XINT(*op0, 1) == UNSPEC_CC_TO_INT && XVECLEN(*op0, 0) == 1 && GET_CODE(XVECEXP(*op0, 0, 0)) == REG && REGNO(XVECEXP(*op0, 0, 0)) == CC_REGNUM && CONST_INT_P(*op1))
  {
    enum rtx_code new_code = UNKNOWN;
    ...
    if (new_code != UNKNOWN)
    {
      /* For CCRAWmode put the required cc mask into the second
         operand.  */
      if (GET_MODE(XVECEXP(*op0, 0, 0)) == CCRAWmode && INTVAL(*op1) >= 0 && INTVAL(*op1) <= 3)
  ...
  if ((*code == NE || *code == EQ) && (GET_CODE(*op0) == EQ || GET_CODE(*op0) == NE) && GET_MODE(*op0) == SImode && GET_MODE(XEXP(*op0, 0)) == CCZ1mode && REG_P(XEXP(*op0, 0)) && XEXP(*op0, 1) == const0_rtx && *op1 == const0_rtx)
  {
    if ((*code == EQ && GET_CODE(*op0) == NE) || (*code == NE && GET_CODE(*op0) == EQ))
      *code = EQ;
    else
      *code = NE;
    *op0 = XEXP(*op0, 0);
  ...
  if ((*code == EQ || *code == NE) && *op1 == const0_rtx && COMPARISON_P(*op0) && CC_REG_P(XEXP(*op0, 0)))
  {
  ...
  /* ~a==b -> ~(a^b)==0   ~a!=b -> ~(a^b)!=0 */
  if (TARGET_Z15 && (*code == EQ || *code == NE) && (GET_MODE(*op0) == DImode || GET_MODE(*op0) == SImode) && GET_CODE(*op0) == NOT)
  {
  ...
  if (TARGET_Z15 && (*code == EQ || *code == NE) && (GET_CODE(*op0) == AND || GET_CODE(*op0) == IOR) && (GET_MODE(*op0) == DImode || GET_MODE(*op0) == SImode) && CONST_INT_P(*op1) && *op1 == constm1_rtx)
  {
    machine_mode mode = GET_MODE(*op0);
...
rtx s390_emit_compare(enum rtx_code code, rtx op0, rtx op1)
{
 ...
 machine modes are compatible.  */
    gcc_assert(s390_cc_modes_compatible(GET_MODE(op0), mode) == GET_MODE(op0));
    cc = op0;
  }
  else
  {
...
int s390_branch_condition_mask(rtx code)
{
  const int CC0 = 1 << 3;
  const int CC1 = 1 << 2;
  const int CC2 = 1 << 1;
  ...
  gcc_assert(XEXP(code, 1) == const0_rtx || (GET_MODE(XEXP(code, 0)) == CCRAWmode && CONST_INT_P(XEXP(code, 1))));
  ...
  case E_CCL1mode:
    ...
    {
    case LTU:
      return CC2 | CC3; /* carry */
    case GEU:
      return CC0 | CC1; /* no carry */
    default:
      return -1;
    }
    break;
  ...
  case E_CCL2mode:
    ...
    {
    case GTU:
      return CC0 | CC1; /* borrow */
    case LEU:
      return CC2 | CC3; /* no borrow */
    default:
      return -1;
    }
    break;
...
int s390_compare_and_branch_condition_mask(rtx code)
{
  const int CC0 = 1 << 3;
  const int CC1 = 1 << 2;
  const int CC2 = 1 << 1;
...
s390_branch_condition_mnemonic(rtx code, int inv)
{
  ...
  if (GET_CODE(XEXP(code, 0)) == REG && REGNO(XEXP(code, 0)) == CC_REGNUM && (XEXP(code, 1) == const0_rtx || (GET_MODE(XEXP(code, 0)) == CCRAWmode && CONST_INT_P(XEXP(code, 1)))))
                     ...
                     int def)
{
  ...
  unsigned HOST_WIDE_INT part_mask = (HOST_WIDE_INT_1U << GET_MODE_BITSIZE(part_mode)) - 1;
  int i, part = -1;
...
bool s390_contiguous_bitmask_nowrap_p(unsigned HOST_WIDE_INT in, int size,
                                      int *pstart, int *pend)
{
  int start;
  int end = -1;
  ...
  if (start > highbit)
  {
    ...
    mask = ((~HOST_WIDE_INT_0U >> highbit) & (~HOST_WIDE_INT_0U << (lowbit - start + 1)));
    if (mask & in)
...
bool s390_contiguous_bitmask_p(unsigned HOST_WIDE_INT in, bool wrap_p,
...
bool s390_contiguous_bitmask_vector_p(rtx op, int *start, int *end)
{
  unsigned HOST_WIDE_INT mask;
  int size;
  rtx elt;
  ...
  gcc_assert(!!start == !!end);
  if (!const_vec_duplicate_p(op, &elt) || !CONST_INT_P(elt))
    return false;
...
bool s390_bytemask_vector_p(rtx op, unsigned *mask)
{
  int i;
  unsigned tmp_mask = 0;
  int nunit, unit_size;

  if (!VECTOR_MODE_P(GET_MODE(op)) || GET_CODE(op) != CONST_VECTOR || !CONST_INT_P(XVECEXP(op, 0, 0)))
    return false;
...
bool s390_extzv_shift_ok(int bitsize, int rotl, unsigned HOST_WIDE_INT contig)
{
  int start, end;
  bool ok;
...
bool s390_split_ok_p(rtx dst, rtx src, machine_mode mode, int first_subword)
{
  ...
  if ((GET_CODE(src) == MEM && !offsettable_memref_p(src)) || (GET_CODE(dst) == MEM && !offsettable_memref_p(dst)))
    return false;
...
bool s390_overlap_p(rtx mem1, rtx mem2, HOST_WIDE_INT size)
{
  ...
  if (delta == 0 || (delta > 0 && delta < size) || (delta < 0 && -delta < size))
    return true;
...
bool s390_offset_p(rtx mem1, rtx mem2, rtx delta)
{
  rtx addr1, addr2, addr_delta;
...
void s390_expand_logical_operator(enum rtx_code code, machine_mode mode,
                                  rtx *operands)
{
  ...
  if (mode != wmode)
  {
    if (GET_CODE(dst) == SUBREG && (tem = simplify_subreg(wmode, dst, mode, 0)) != 0)
      dst = tem;
    else if (REG_P(dst))
    ...
    if (GET_CODE(src1) == SUBREG && (tem = simplify_subreg(wmode, src1, mode, 0)) != 0)
      src1 = tem;
    else if (GET_MODE(src1) != VOIDmode)
      src1 = gen_rtx_SUBREG(wmode, force_reg(mode, src1), 0);

    if (GET_CODE(src2) == SUBREG && (tem = simplify_subreg(wmode, src2, mode, 0)) != 0)
      src2 = tem;
...
bool s390_logical_operator_ok_p(rtx *operands)
{
  ...
  if (GET_CODE(operands[0]) == MEM)
    return rtx_equal_p(operands[0], operands[1]) || (!reload_completed && rtx_equal_p(operands[0], operands[2]));
...
void s390_narrow_logical_operator(enum rtx_code code, rtx *memop, rtx *immop)
{
...
s390_short_displacement(rtx disp)
{
  ...
  if (GET_CODE(disp) == CONST && GET_CODE(XEXP(disp, 0)) == UNSPEC && (XINT(XEXP(disp, 0), 1) == UNSPEC_GOT || XINT(XEXP(disp, 0), 1) == UNSPEC_GOTNTPOFF))
    return false;
                                 ...
                                 bool *is_base_ptr, bool *is_pool_ptr)
{
  ...
  if (REGNO(*ref) == STACK_POINTER_REGNUM || REGNO(*ref) == FRAME_POINTER_REGNUM || ((reload_completed || reload_in_progress) && frame_pointer_needed && REGNO(*ref) == HARD_FRAME_POINTER_REGNUM) || REGNO(*ref) == ARG_POINTER_REGNUM || (flag_pic && REGNO(*ref) == PIC_OFFSET_TABLE_REGNUM))
    *is_ptr = *is_base_ptr = true;

  if ((reload_completed || reload_in_progress) && *ref == cfun->machine->base_reg)
...
s390_decompose_address(rtx addr, struct s390_address *out)
{
  ...
  if (disp)
  {
    ...
    else if (GET_CODE(disp) == CONST && GET_CODE(XEXP(disp, 0)) == PLUS && GET_CODE(XEXP(XEXP(disp, 0), 1)) == CONST_INT)
    {
  ...
  if (base && indx && !base_ptr && (indx_ptr || (!REG_POINTER(base) && REG_POINTER(indx))))
  {
    rtx tmp = base;
    base = indx;
    indx = tmp;
 ...
 final elimination.  */
    if (lra_in_progress || (base != arg_pointer_rtx && indx != arg_pointer_rtx && base != return_address_pointer_rtx && indx != return_address_pointer_rtx && base != frame_pointer_rtx && indx != frame_pointer_rtx && base != virtual_stack_vars_rtx && indx != virtual_stack_vars_rtx))
 ...
 and @GOTNTPOFF offsets to possible displacements.  */
    if (GET_CODE(disp) == UNSPEC && (XINT(disp, 1) == UNSPEC_GOT || XINT(disp, 1) == UNSPEC_GOTNTPOFF) && flag_pic == 1)
    {
      ;
    }

    /* Accept pool label offsets.  */
    else if (GET_CODE(disp) == UNSPEC && XINT(disp, 1) == UNSPEC_POOL_OFFSET)
      ;

    /* Accept literal pool references.  */
    else if (GET_CODE(disp) == UNSPEC && XINT(disp, 1) == UNSPEC_LTREL_OFFSET)
    {
      ...
      if (GET_CODE(XVECEXP(disp, 0, 0)) != SYMBOL_REF || !CONSTANT_POOL_ADDRESS_P(XVECEXP(disp, 0, 0)))
        return false;
...
bool s390_decompose_addrstyle_without_index(rtx op, rtx *base,
...
/*  Check that OP is a valid shift count operand.
  ...
  */
bool s390_valid_shift_count(rtx op, HOST_WIDE_INT implicit_mask)
{
  /* Strip subreg.  */
  ...
  if (GET_CODE(op) == AND)
  {
    ...
    if (implicit_mask > 0 && (val & implicit_mask) != implicit_mask)
      return false;
...
bool s390_legitimate_address_without_index_p(rtx op)
{
...
s390_loadrelative_operand_p(rtx addr, rtx *symref, HOST_WIDE_INT *addend)
{
  ...
  if (GET_CODE(addr) == SYMBOL_REF || (GET_CODE(addr) == UNSPEC && (XINT(addr, 1) == UNSPEC_GOTENT || XINT(addr, 1) == UNSPEC_PLT31)))
  {
    if (symref)
      *symref = addr;
    if (addend)
...
s390_check_qrst_address(char c, rtx op, bool lit_pool_ok)
{
  ...
  if (s390_loadrelative_operand_p(op, &symref, NULL) && (!lit_pool_ok || !SYMBOL_REF_P(symref) || !CONSTANT_POOL_ADDRESS_P(symref)))
    return 0;
...
int s390_mem_constraint(const char *str, rtx op)
{
  char c = str[0];
...
int s390_O_constraint_str(const char c, HOST_WIDE_INT value)
{
  if (!TARGET_EXTIMM)
    return 0;
  ...
  case 'p':
    return value == 0 || s390_single_part(GEN_INT(value), DImode, SImode, 0) == 1;
...
int s390_N_constraint_str(const char *str, HOST_WIDE_INT value)
{
...
int s390_float_const_zero_p(rtx value)
{
  return (GET_MODE_CLASS(GET_MODE(value)) == MODE_FLOAT && value == CONST0_RTX(GET_MODE(value)));
}
                        ...
                        reg_class_t from, reg_class_t to)
{
  ...
  if (TARGET_Z10 && GET_MODE_SIZE(mode) == 8)
  {
    ...
    if (reg_classes_intersect_p(from, GENERAL_REGS) && reg_classes_intersect_p(to, FP_REGS))
      return 1;
    /* lgdr needs 3 cycles. */
    if (reg_classes_intersect_p(to, GENERAL_REGS) && reg_classes_intersect_p(from, FP_REGS))
      return 3;
  }

  /* Otherwise copying is done via memory.  */
  if ((reg_classes_intersect_p(from, GENERAL_REGS) && reg_classes_intersect_p(to, FP_REGS)) || (reg_classes_intersect_p(from, FP_REGS) && reg_classes_intersect_p(to, GENERAL_REGS)))
    return 10;

  /* We usually do not want to copy via CC.  */
  if (reg_classes_intersect_p(from, CC_REGS) || reg_classes_intersect_p(to, CC_REGS))
    return 5;
               ...
               int *total, bool speed ATTRIBUTE_UNUSED)
{
  ...
  case SET:
  {
    ...
    *total = COSTS_N_INSNS(1) + 2;
    ...
    if (!TARGET_Z15 && reload_completed && !rtx_equal_p(dst, then) && !rtx_equal_p(dst, els))
      *total += COSTS_N_INSNS(1) / 2;

    /* A minor penalty for constants we cannot directly handle.  */
    if ((CONST_INT_P(then) || CONST_INT_P(els)) && (!TARGET_Z13 || MEM_P(dst) || (CONST_INT_P(then) && !satisfies_constraint_K(then)) || (CONST_INT_P(els) && !satisfies_constraint_K(els))))
      *total += COSTS_N_INSNS(1) / 2;
  ...
  case IOR:
    ...
    /* nnrk, nngrk */
    if (TARGET_Z15 && (mode == SImode || mode == DImode) && GET_CODE(XEXP(x, 0)) == NOT && GET_CODE(XEXP(x, 1)) == NOT)
    {
    ...
    /* risbg */
    if (GET_CODE(XEXP(x, 0)) == AND && GET_CODE(XEXP(x, 1)) == ASHIFT && REG_P(XEXP(XEXP(x, 0), 0)) && REG_P(XEXP(XEXP(x, 1), 0)) && CONST_INT_P(XEXP(XEXP(x, 0), 1)) && CONST_INT_P(XEXP(XEXP(x, 1), 1)) && (UINTVAL(XEXP(XEXP(x, 0), 1)) == (HOST_WIDE_INT_1U << UINTVAL(XEXP(XEXP(x, 1), 1))) - 1))
    {
 ...
 instruction.  */
    if (TARGET_VXE && GET_CODE(XEXP(x, 0)) == NOT && GET_CODE(XEXP(x, 1)) == NOT && REG_P(XEXP(XEXP(x, 0), 0)) && REG_P(XEXP(XEXP(x, 1), 0)) && GET_MODE_SIZE(GET_MODE(XEXP(XEXP(x, 0), 0))) == 16 && s390_hard_regno_mode_ok(VR0_REGNUM, GET_MODE(XEXP(XEXP(x, 0), 0))))
    {
      *total = COSTS_N_INSNS(1);
      return true;
    }
  ...
  case MULT:
    ...
    case E_SImode:
    {
      rtx left = XEXP(x, 0);
      rtx right = XEXP(x, 1);
      if (GET_CODE(right) == CONST_INT && CONST_OK_FOR_K(INTVAL(right)))
        *total = s390_cost->mhi;
    ...
    case E_DImode:
    {
      ...
      {
        if (GET_CODE(left) == SIGN_EXTEND && GET_CODE(right) == SIGN_EXTEND)
          /* mulsidi case: mr, m */
          *total = s390_cost->m;
        else if (GET_CODE(left) == ZERO_EXTEND && GET_CODE(right) == ZERO_EXTEND)
  ...
  case FMA:
    ...
    {
      *total += (rtx_cost(XEXP(x, 0), mode, FMA, 0, speed) + rtx_cost(XEXP(x, 1), mode, FMA, 1, speed) + rtx_cost(XEXP(XEXP(x, 2), 0), mode, FMA, 2, speed));
      return true;
    }
    return false;
  ...
  case SIGN_EXTEND:
  case ZERO_EXTEND:
    if (outer_code == MULT || outer_code == DIV || outer_code == MOD || outer_code == PLUS || outer_code == MINUS || outer_code == COMPARE)
      *total = 0;
    return false;
  ...
  case COMPARE:
    *total = COSTS_N_INSNS(1);
    ...
    /* nxrk, nxgrk ~(a^b)==0 */
    if (TARGET_Z15 && GET_CODE(XEXP(x, 0)) == NOT && XEXP(x, 1) == const0_rtx && GET_CODE(XEXP(XEXP(x, 0), 0)) == XOR && (GET_MODE(XEXP(x, 0)) == SImode || GET_MODE(XEXP(x, 0)) == DImode) && mode == CCZmode)
    {
    ...
    if (TARGET_Z15 && (GET_CODE(XEXP(x, 0)) == AND || GET_CODE(XEXP(x, 0)) == IOR) && XEXP(x, 1) == const0_rtx && (GET_MODE(XEXP(x, 0)) == SImode || GET_MODE(XEXP(x, 0)) == DImode) && GET_CODE(XEXP(XEXP(x, 0), 0)) == NOT && GET_CODE(XEXP(XEXP(x, 0), 1)) == NOT && mode == CCZmode)
    {
      if (!REG_P(XEXP(XEXP(XEXP(x, 0), 0), 0)))
        *total += 1;
    ...
    if (GET_CODE(XEXP(x, 0)) == AND && GET_CODE(XEXP(x, 1)) == CONST_INT && GET_CODE(XEXP(XEXP(x, 0), 1)) == CONST_INT)
    {
      rtx op0 = XEXP(XEXP(x, 0), 0);
      rtx op1 = XEXP(XEXP(x, 0), 1);
      rtx op2 = XEXP(x, 1);

      if (memory_operand(op0, GET_MODE(op0)) && s390_tm_ccmode(op1, op2, 0) != VOIDmode)
        return true;
      if (register_operand(op0, GET_MODE(op0)) && s390_tm_ccmode(op1, op2, 1) != VOIDmode)
        return true;
    }
    return false;
...
void s390_split_access_reg(rtx reg, rtx *lo, rtx *hi)
{
...
bool tls_symbolic_reference_mentioned_p(rtx op)
{
  const char *fmt;
  int i;
...
int legitimate_pic_operand_p(rtx op)
{
...
s390_legitimate_constant_p(machine_mode mode, rtx op)
{
  if (TARGET_VX && VECTOR_MODE_P(mode) && GET_CODE(op) == CONST_VECTOR)
  {
    ...
    if (!satisfies_constraint_j00(op) && !satisfies_constraint_jm1(op) && !satisfies_constraint_jKK(op) && !satisfies_constraint_jxx(op) && !satisfies_constraint_jyy(op))
      return 0;
  }
...
s390_cannot_force_const_mem(machine_mode mode, rtx x)
{
 ...
 non-TLS symbols are OK iff we are non-PIC.  */
  ...
  case PLUS:
  case MINUS:
    return s390_cannot_force_const_mem(mode, XEXP(x, 0)) || s390_cannot_force_const_mem(mode, XEXP(x, 1));
...
bool legitimate_reload_constant_p(rtx op)
{
  /* Accept la(y) operands.  */
  if (GET_CODE(op) == CONST_INT && DISP_IN_RANGE(INTVAL(op)))
    return true;

  /* Accept l(g)hi/l(g)fi operands.  */
  if (GET_CODE(op) == CONST_INT && (CONST_OK_FOR_K(INTVAL(op)) || CONST_OK_FOR_Os(INTVAL(op))))
    return true;

  /* Accept lliXX operands.  */
  if (TARGET_ZARCH && GET_CODE(op) == CONST_INT && trunc_int_for_mode(INTVAL(op), word_mode) == INTVAL(op) && s390_single_part(op, word_mode, HImode, 0) >= 0)
    return true;

  if (TARGET_EXTIMM && GET_CODE(op) == CONST_INT && trunc_int_for_mode(INTVAL(op), word_mode) == INTVAL(op) && s390_single_part(op, word_mode, SImode, 0) >= 0)
    return true;
  ...
  if (GET_CODE(op) == CONST_DOUBLE && s390_float_const_zero_p(op) && GET_MODE_SIZE(GET_MODE(op)) <= UNITS_PER_WORD)
    return true;

  /* Accept double-word operands that can be split.  */
  if (GET_CODE(op) == CONST_WIDE_INT || (GET_CODE(op) == CONST_INT && trunc_int_for_mode(INTVAL(op), word_mode) != INTVAL(op)))
  {
    ...
    return legitimate_reload_constant_p(hi) && legitimate_reload_constant_p(lo);
  }
...
legitimate_reload_fp_constant_p(rtx op)
{
  ...
  if (TARGET_Z196 && GET_CODE(op) == CONST_DOUBLE && s390_float_const_zero_p(op))
    return true;
...
{
  if (TARGET_VX && GET_MODE_SIZE(GET_MODE(op)) == 16 && (satisfies_constraint_j00(op) || satisfies_constraint_jm1(op) || satisfies_constraint_jKK(op) || satisfies_constraint_jxx(op) || satisfies_constraint_jyy(op)))
    return true;
...
must be forced into the literal pool.  */
  ...
  case CONST_INT:
  case CONST_WIDE_INT:
    if (reg_class_subset_p(GENERAL_REGS, rclass) && legitimate_reload_constant_p(op))
      return GENERAL_REGS;
    else if (reg_class_subset_p(ADDR_REGS, rclass) && legitimate_reload_constant_p(op))
      return ADDR_REGS;
    else if (reg_class_subset_p(FP_REGS, rclass) && legitimate_reload_fp_constant_p(op))
      return FP_REGS;
    else if (reg_class_subset_p(VEC_REGS, rclass) && legitimate_reload_vector_constant_p(op))
      return VEC_REGS;
...
of ADDR_REGS, e.g. FP_REGS, reject this reload.  */
  case CONST:
    ...
    if (GET_CODE(XEXP(op, 0)) == PLUS && GET_CODE(XEXP(XEXP(op, 0), 0)) == SYMBOL_REF && GET_CODE(XEXP(XEXP(op, 0), 1)) == CONST_INT)
    {
...
bool s390_check_symref_alignment(rtx addr, HOST_WIDE_INT alignment)
{
 ...
 SYMBOL_FLAG_NOTALIGN{2,4,8}_P returning useful information.  */
    if (SYMBOL_REF_ANCHOR_P(symref))
    {
      ...
      unsigned int block_alignment = (SYMBOL_REF_BLOCK(symref)->alignment / BITS_PER_UNIT);

      gcc_assert(block_offset >= 0);
      return ((block_offset & (alignment - 1)) == 0 && block_alignment >= alignment);
    }
 ...
 8-byte alignment so allow only these.  */
    ...
    {
    case 8:
      return !SYMBOL_FLAG_NOTALIGN8_P(symref);
    case 4:
      return !SYMBOL_FLAG_NOTALIGN4_P(symref);
    case 2:
      return !SYMBOL_FLAG_NOTALIGN2_P(symref);
    default:
      return false;
    }
  }

  if (GET_CODE(symref) == UNSPEC && alignment <= UNITS_PER_LONG)
    return true;
...
void s390_reload_larl_operand(rtx reg, rtx addr, rtx scratch)
{
  HOST_WIDE_INT addend;
  rtx symref;
 ...
 register is a valid base register.  */
    if (REGNO(reg) < FIRST_PSEUDO_REGISTER && REGNO_REG_CLASS(REGNO(reg)) == ADDR_REGS)
      scratch = reg;
...
void s390_reload_symref_address(rtx reg, rtx mem, rtx scratch, bool tomem)
{
  /* Reload might have pulled a constant out of the literal pool.
     Force it back in.  */
  if (CONST_INT_P(mem) || GET_CODE(mem) == CONST_DOUBLE || GET_CODE(mem) == CONST_WIDE_INT || GET_CODE(mem) == CONST_VECTOR || GET_CODE(mem) == CONST)
  ...
  if (!tomem && REGNO(reg) < FIRST_PSEUDO_REGISTER && REGNO_REG_CLASS(REGNO(reg)) == ADDR_REGS && GET_MODE(reg) == GET_MODE(scratch))
    scratch = reg;
                      ...
                      machine_mode mode, secondary_reload_info *sri)
{
 ...
 displacements.  */
    if (MEM_P(x) && GET_CODE(XEXP(x, 0)) == PLUS && GET_CODE(XEXP(XEXP(x, 0), 1)) == CONST_INT && !SHORT_DISP_IN_RANGE(INTVAL(XEXP(XEXP(x, 0), 1))) && reg_class_subset_p(rclass, VEC_REGS) && (!reg_class_subset_p(rclass, FP_REGS) || (GET_MODE_SIZE(mode) > 8 && s390_class_max_nregs(FP_REGS, mode) == 1)))
    {
      if (in_p)
        sri->icode = (TARGET_64BIT ? CODE_FOR_reloaddi_la_in : CODE_FOR_reloadsi_la_in);
      else
        sri->icode = (TARGET_64BIT ? CODE_FOR_reloaddi_la_out : CODE_FOR_reloadsi_la_out);
    }
  }
 ...
 an odd addend.  */
    if (in_p && s390_loadrelative_operand_p(x, &symref, &offset) && mode == Pmode && !SYMBOL_FLAG_NOTALIGN2_P(symref) && (offset & 1) == 1)
      sri->icode = ((mode == DImode) ? CODE_FOR_reloaddi_larl_odd_addend_z10
                                     : CODE_FOR_reloadsi_larl_odd_addend_z10);

    /* Handle all the (mem (symref)) accesses we cannot use the z10
 instructions for.  */
    if (MEM_P(x) && s390_loadrelative_operand_p(XEXP(x, 0), NULL, NULL) && (mode == QImode || !reg_class_subset_p(rclass, GENERAL_REGS) || GET_MODE_SIZE(mode) > UNITS_PER_WORD || !s390_check_symref_alignment(XEXP(x, 0), GET_MODE_SIZE(mode))))
    {
#define __SECONDARY_RELOAD_CASE(M, m)                                                          \
  case E_##M##mode:                                                                            \
    if (TARGET_64BIT)                                                                          \
      sri->icode = in_p ? CODE_FOR_reload##m##di_toreg_z10 : CODE_FOR_reload##m##di_tomem_z10; \
    else                                                                                       \
      sri->icode = in_p ? CODE_FOR_reload##m##si_toreg_z10 : CODE_FOR_reload##m##si_tomem_z10; \
    break;
...
#undef __SECONDARY_RELOAD_CASE
  ...
  if (!lra_in_progress && in_p && s390_plus_operand(x, mode))
    sri->icode = (TARGET_64BIT ? CODE_FOR_reloaddi_plus : CODE_FOR_reloadsi_plus);
  ...
  /* Performing a multiword move from or to memory we have to make sure the
     ...
     a scratch register.  */
  if (MEM_P(x) && GET_CODE(XEXP(x, 0)) == PLUS && GET_CODE(XEXP(XEXP(x, 0), 1)) == CONST_INT && !DISP_IN_RANGE(INTVAL(XEXP(XEXP(x, 0), 1)) + GET_MODE_SIZE(mode) - 1))
  {
 ...
 have to care about overflows in the b+i+d case.  */
    if ((reg_classes_intersect_p(GENERAL_REGS, rclass) && s390_class_max_nregs(GENERAL_REGS, mode) > 1 && GET_CODE(XEXP(XEXP(x, 0), 0)) == PLUS)
        /* For FP_REGS no lm/stm is available so this check is triggered
           for displacement overflows in b+i+d and b+d like addresses.  */
        || (reg_classes_intersect_p(FP_REGS, rclass) && s390_class_max_nregs(FP_REGS, mode) > 1))
    {
      if (in_p)
        sri->icode = (TARGET_64BIT ? CODE_FOR_reloaddi_la_in : CODE_FOR_reloadsi_la_in);
      else
...
{
  return (((reg_classes_intersect_p(class1, VEC_REGS) && reg_classes_intersect_p(class2, GENERAL_REGS)) || (reg_classes_intersect_p(class1, GENERAL_REGS) && reg_classes_intersect_p(class2, VEC_REGS))) && (TARGET_TPF || !TARGET_DFP || !TARGET_64BIT || GET_MODE_SIZE(mode) != 8) && (!TARGET_VX || (SCALAR_FLOAT_MODE_P(mode) && GET_MODE_SIZE(mode) > 8)));
}
...
void s390_expand_plus_operand(rtx target, rtx src,
                              rtx scratch)
{
  rtx sum1, sum2;
  struct s390_address ad;
  ...
  if (!s390_decompose_address(src, &ad) || (ad.base && !REGNO_OK_FOR_BASE_P(REGNO(ad.base))) || (ad.indx && !REGNO_OK_FOR_INDEX_P(REGNO(ad.indx))))
  {
...
s390_legitimate_address_p(machine_mode mode, rtx addr, bool strict)
{
  ...
  if (TARGET_Z10 && larl_operand(addr, VOIDmode) && (mode == VOIDmode || s390_check_symref_alignment(addr, GET_MODE_SIZE(mode))))
    return true;
  ...
  /* The vector memory instructions only support short displacements.
     ...
     properly.  */
  if (TARGET_VX && VECTOR_MODE_P(mode) && ad.disp != NULL_RTX && CONST_INT_P(ad.disp) && !SHORT_DISP_IN_RANGE(INTVAL(ad.disp)))
    return false;
  ...
  {
    if (ad.base && !(REGNO(ad.base) >= FIRST_PSEUDO_REGISTER || REGNO_REG_CLASS(REGNO(ad.base)) == ADDR_REGS))
      return false;

    if (ad.indx && !(REGNO(ad.indx) >= FIRST_PSEUDO_REGISTER || REGNO_REG_CLASS(REGNO(ad.indx)) == ADDR_REGS))
      return false;
  }
  return true;
}
...
bool legitimate_la_operand_p(rtx op)
{
  struct s390_address addr;
  if (!s390_decompose_address(op, &addr))
    return false;
...
bool preferred_la_operand_p(rtx op1, rtx op2)
{
  struct s390_address addr;
...
void s390_load_address(rtx dst, rtx src)
{
  if (TARGET_64BIT)
...
bool s390_rel_address_ok_p(rtx symbol_ref)
{
  tree decl;
  ...
  if (!flag_pic || SYMBOL_REF_LOCAL_P(symbol_ref))
    return (s390_pic_data_is_text_relative || (decl && TREE_CODE(decl) == FUNCTION_DECL));
...
rtx legitimize_pic_address(rtx orig, rtx reg)
{
  ...
  if ((GET_CODE(addr) == LABEL_REF || (SYMBOL_REF_P(addr) && s390_rel_address_ok_p(addr)) || (GET_CODE(addr) == UNSPEC && (XINT(addr, 1) == UNSPEC_GOTENT || XINT(addr, 1) == UNSPEC_PLT31))) && GET_CODE(addend) == CONST_INT)
  {
    /* This can be locally addressed.  */

    /* larl_operand requires UNSPECs to be wrapped in a const rtx.  */
    rtx const_addr = (GET_CODE(addr) == UNSPEC ? gen_rtx_CONST(Pmode, addr) : addr);

    if (larl_operand(const_addr, VOIDmode) && INTVAL(addend) < HOST_WIDE_INT_1 << 31 && INTVAL(addend) >= -(HOST_WIDE_INT_1 << 31))
    {
      if (INTVAL(addend) & 1)
      {
 ...
 that case.  So no need to do it here.  */
    ...
    else
    {
      ...
      gcc_assert(REGNO(temp) >= FIRST_PSEUDO_REGISTER || REGNO_REG_CLASS(REGNO(temp)) == ADDR_REGS);
...
rtx s390_get_thread_pointer(void)
{
...
legitimize_tls_address(rtx addr, rtx reg)
{
  ...
  else if (GET_CODE(addr) == CONST && GET_CODE(XEXP(addr, 0)) == PLUS && GET_CODE(XEXP(XEXP(addr, 0), 1)) == CONST_INT)
  {
...
void emit_symbolic_move(rtx *operands)
{
                        ...
                        machine_mode mode ATTRIBUTE_UNUSED)
{
  ...
  else if (GET_CODE(x) == PLUS && (TLS_SYMBOLIC_CONST(XEXP(x, 0)) || TLS_SYMBOLIC_CONST(XEXP(x, 1))))
  {
    return x;
  }
  else if (flag_pic)
  {
    if (SYMBOLIC_CONST(x) || (GET_CODE(x) == PLUS && (SYMBOLIC_CONST(XEXP(x, 0)) || SYMBOLIC_CONST(XEXP(x, 1)))))
  ...
  if (GET_CODE(constant_term) == CONST_INT && !TARGET_LONG_DISPLACEMENT && !DISP_IN_RANGE(INTVAL(constant_term)) && !(REG_P(x) && REGNO_PTR_FRAME_P(REGNO(x))))
  {
...
rtx legitimize_reload_address(rtx ad, machine_mode mode ATTRIBUTE_UNUSED,
                              int opnum, int type)
{
  ...
  if (GET_CODE(ad) == PLUS && GET_CODE(XEXP(ad, 0)) == REG && GET_CODE(XEXP(ad, 1)) == CONST_INT && !DISP_IN_RANGE(INTVAL(XEXP(ad, 1))))
  {
...
bool s390_expand_cpymem(rtx dst, rtx src, rtx len)
{
  ...
  if (s390_tune >= PROCESSOR_2097_Z10 && (GET_CODE(len) != CONST_INT || INTVAL(len) > (1 << 16)))
    return false;
  ...
  if (GET_CODE(len) == CONST_INT && INTVAL(len) >= 0 && INTVAL(len) <= 256 * 6 && (!TARGET_MVCLE || INTVAL(len) <= 256))
  {
    HOST_WIDE_INT o, l;
  ...
  else
  {
    ...
    if (TARGET_Z10 && (GET_CODE(len) != CONST_INT || INTVAL(len) > 768))
    {
      rtx prefetch;
...
void s390_expand_setmem(rtx dst, rtx len, rtx val)
{
  ...
  if (GET_CODE(len) == CONST_INT && ((val == const0_rtx && (INTVAL(len) <= 256 * 4 || (INTVAL(len) <= 256 * 5 && TARGET_SETMEM_PFD(val, len)))) || (val != const0_rtx && INTVAL(len) <= 257 * 4)) && (!TARGET_MVCLE || INTVAL(len) <= 256))
  {
    HOST_WIDE_INT o, l;
...
bool s390_expand_cmpmem(rtx target, rtx op0, rtx op1, rtx len)
{
  ...
  else
  {
    ...
    if (TARGET_Z10 && (GET_CODE(len) != CONST_INT || INTVAL(len) > 512))
    {
      rtx prefetch;
...
void s390_expand_vec_strlen(rtx target, rtx string, rtx alignment)
{
...
void s390_expand_vec_movstr(rtx result, rtx dst, rtx src)
{
  rtx temp = gen_reg_rtx(Pmode);
  ...
  add_int_reg_note(s390_emit_ccraw_jump(8, EQ, done_label),
                   REG_BR_PROB, profile_probability::very_unlikely().to_reg_br_prob_note());
...
bool s390_expand_addcc(enum rtx_code cmp_code, rtx cmp_op0, rtx cmp_op1,
                       rtx dst, rtx src, rtx increment)
{
  ...
  if ((GET_MODE(cmp_op0) == SImode || GET_MODE(cmp_op0) == VOIDmode) && (GET_MODE(cmp_op1) == SImode || GET_MODE(cmp_op1) == VOIDmode))
    cmp_mode = SImode;
  else if ((GET_MODE(cmp_op0) == DImode || GET_MODE(cmp_op0) == VOIDmode) && (GET_MODE(cmp_op1) == DImode || GET_MODE(cmp_op1) == VOIDmode))
    cmp_mode = DImode;
  else
    return false;
...
bool s390_expand_insv(rtx dest, rtx op1, rtx op2, rtx src)
{
  ...
  /* Just a move.  */
  if (bitpos == 0 && bitsize == GET_MODE_BITSIZE(GET_MODE(src)) && mode == GET_MODE(src))
  {
  ...
  if (TARGET_ZARCH && register_operand(dest, word_mode) && (bitpos % 16) == 0 && (bitsize % 16) == 0 && const_int_operand(src, VOIDmode))
  {
  ...
  if (bitpos == 0 && (bitsize % BITS_PER_UNIT) == 0 && MEM_P(dest) && (register_operand(src, word_mode) || const_int_operand(src, VOIDmode)))
  {
  ...
  if ((bitpos % BITS_PER_UNIT) == 0 && (bitsize % BITS_PER_UNIT) == 0 && (bitpos & 32) == ((bitpos + bitsize - 1) & 32) && MEM_P(src) && (mode == DImode || mode == SImode) && mode != smode && register_operand(dest, mode))
  {
    ...
    if (smode_bsize == bitsize && bitpos == mode_bsize - smode_bsize)
    {
      ...
      {
      case E_QImode:
        emit_insn(gen_movstrictqi(low_dest, low_src));
        return true;
      case E_HImode:
        emit_insn(gen_movstricthi(low_dest, low_src));
        return true;
      case E_SImode:
        emit_insn(gen_movstrictsi(low_dest, low_src));
        return true;
      default:
        break;
      }
...
void s390_expand_vec_compare(rtx target, enum rtx_code cond,
...
/* Expand the comparison CODE of CMP1 and CMP2 and copy 1 or 0 into
   ...
   builtins.  */
void s390_expand_vec_compare_cc(rtx target, enum rtx_code code,
 ...
 that.  */
  ...
  else if (GET_MODE_CLASS(GET_MODE(cmp1)) == MODE_VECTOR_FLOAT)
  {
    ...
    case NE:
      ...
      break;
    case GT:
      cc_producer_mode = CCVFHmode;
      break;
    case GE:
      cc_producer_mode = CCVFHEmode;
      break;
    case UNLE:
      cc_producer_mode = CCVFHmode;
      inv_p = true;
  ...
  emit_insn(gen_rtx_PARALLEL(VOIDmode,
                             gen_rtvec(2, gen_rtx_SET(gen_rtx_REG(cc_producer_mode, CC_REGNUM), gen_rtx_COMPARE(cc_producer_mode, cmp1, cmp2)),
...
void s390_expand_vcond(rtx target, rtx then, rtx els,
                       enum rtx_code cond, rtx cmp_op1, rtx cmp_op2)
{
  ...
  if ((cond == LT || cond == GE) && target_mode == cmp_mode && cmp_op2 == CONST0_RTX(cmp_mode) && op == CONST0_RTX(target_mode) && s390_vector_mode_supported_p(target_mode) && GET_MODE_CLASS(target_mode) == MODE_VECTOR_INT)
  {
  ...
  if (all_ones_operand(then, GET_MODE(then)) && const0_operand(els, GET_MODE(els)))
  {
...
/* Emit the RTX necessary to initialize the vector TARGET with values
   in VALS.  */
void s390_expand_vec_init(rtx target, rtx vals)
{
  machine_mode mode = GET_MODE(target);
  ...
  if (all_same && all_const_int)
  {
    ...
    if (XVECEXP(vals, 0, 0) == const0_rtx || s390_contiguous_bitmask_vector_p(vec, NULL, NULL) || s390_bytemask_vector_p(vec, NULL))
    {
  ...
  if (all_regs && REG_P(target) && n_elts == 2 && GET_MODE_SIZE(inner_mode) == 8)
  {
...
/* Return a parallel of constant integers to be used as permutation
   ...
   right-most elements.  */
rtx s390_expand_merge_perm_const(machine_mode mode, bool high_p)
{
...
void s390_expand_merge(rtx target, rtx src1, rtx src2, bool high_p)
{
  machine_mode mode = GET_MODE(target);
                   ...
                   rtx cmp, rtx new_rtx, bool is_weak)
{
 ...
 branch so that we can generate CRJ.  */
  ...
  convert_move(vtarget, expand_simple_binop(SImode, LSHIFTRT, res, ac.shift, NULL_RTX, 1, OPTAB_DIRECT), 1);
}
                    ...
                    rtx cmp, rtx new_rtx, bool is_weak)
{
  ...
  if (TARGET_Z196 && (mode == SImode || mode == DImode))
  ...
  if (do_const_opt)
  {
    ...
    if (CONST_INT_P(cmp) && INTVAL(cmp) == 0)
    {
      ...
      RTVEC_ELT(lt, 0) = gen_rtx_SET(cc, gen_rtx_COMPARE(CCZmode, mem, cmp));
...
void s390_expand_cs(machine_mode mode, rtx btarget, rtx vtarget, rtx mem,
                    rtx cmp, rtx new_rtx, bool is_weak)
{
  switch (mode)
  {
...
void s390_expand_atomic_exchange_tdsi(rtx output, rtx mem, rtx input)
{
  machine_mode mode = GET_MODE(mem);
  rtx_code_label *csloop;

  if (TARGET_Z196 && (mode == DImode || mode == SImode) && CONST_INT_P(input) && INTVAL(input) == 0)
  {
...
void s390_expand_atomic(machine_mode mode, enum rtx_code code,
                        rtx target, rtx mem, rtx val, bool after)
{
  ...
  /* Return the correct part of the bitfield.  */
  if (target)
    convert_move(target, expand_simple_binop(SImode, LSHIFTRT, after ? new_rtx : cmp, ac.shift, NULL_RTX, 1, OPTAB_DIRECT), 1);
}
...
s390_mangle_type(const_tree type)
{
  ...
  if (TREE_CODE(type) != VOID_TYPE && TREE_CODE(type) != BOOLEAN_TYPE && TREE_CODE(type) != INTEGER_TYPE && TREE_CODE(type) != REAL_TYPE)
    return NULL;

  if (type == s390_builtin_types[BT_BV16QI])
    return "U6__boolc";
  if (type == s390_builtin_types[BT_BV8HI])
    return "U6__bools";
  if (type == s390_builtin_types[BT_BV4SI])
    return "U6__booli";
  if (type == s390_builtin_types[BT_BV2DI])
    return "U6__booll";

  if (TYPE_MAIN_VARIANT(type) == long_double_type_node && TARGET_LONG_DOUBLE_128)
    return "g";
...
s390_delegitimize_address(rtx orig_x)
{
  ...
  /* Extract the symbol ref from:
     ...
     (plus:SI (reg:SI 12 %r12)
        (const:SI (plus:SI (unspec:SI [(symbol_ref:SI ("L"))]
               UNSPEC_GOTOFF/PLTOFF)
         (const_int 4 [0x4]))))  */
  if (GET_CODE(x) == PLUS && REG_P(XEXP(x, 0)) && REGNO(XEXP(x, 0)) == PIC_OFFSET_TABLE_REGNUM && GET_CODE(XEXP(x, 1)) == CONST)
  {
    HOST_WIDE_INT offset = 0;
    ...
    if (GET_CODE(y) == PLUS && GET_CODE(XEXP(y, 1)) == CONST_INT)
    {
      offset = INTVAL(XEXP(y, 1));
      y = XEXP(y, 0);
    }

    if (GET_CODE(y) == UNSPEC && (XINT(y, 1) == UNSPEC_GOTOFF || XINT(y, 1) == UNSPEC_PLTOFF))
  ...
  x = XEXP(x, 0);
  if (GET_CODE(x) == PLUS && GET_CODE(XEXP(x, 1)) == CONST && GET_CODE(XEXP(x, 0)) == REG && REGNO(XEXP(x, 0)) == PIC_OFFSET_TABLE_REGNUM)
  {
    y = XEXP(XEXP(x, 1), 0);
    if (GET_CODE(y) == UNSPEC && XINT(y, 1) == UNSPEC_GOT)
      y = XVECEXP(y, 0, 0);
    else
      return orig_x;
  }
 ...
 (mem:QI (const:DI (unspec:DI [(symbol_ref:DI ("foo"))]
    ...
    y = XEXP(x, 0);
    if (GET_CODE(y) == UNSPEC && (XINT(y, 1) == UNSPEC_GOTENT || XINT(y, 1) == UNSPEC_PLT31))
      y = XVECEXP(y, 0, 0);
    else
      return orig_x;
  }
...
void s390_asm_output_function_prefix(FILE *asm_out_file,
...
void s390_asm_declare_function_size(FILE *asm_out_file,
...
void s390_asm_output_function_label(FILE *out_file, const char *fname,
                                    tree decl)
{
...
void print_operand_address(FILE *file, rtx addr)
{
  ...
  if (!s390_decompose_address(addr, &ad) || (ad.base && !REGNO_OK_FOR_BASE_P(REGNO(ad.base))) || (ad.indx && !REGNO_OK_FOR_INDEX_P(REGNO(ad.indx))))
...
void print_operand(FILE *file, rtx x, int code)
{
  ...
  case 'O':
  {
    ...
    if (!ret || (ad.base && !REGNO_OK_FOR_BASE_P(REGNO(ad.base))) || ad.indx)
    {
 ...
 a non-NULL result, so skip them here as well.  */
    if (TARGET_64BIT && GET_CODE(x) == SYMBOL_REF && SYMBOL_REF_FUNCTION_P(x) && !(SYMBOL_REF_WEAK(x) && !flag_pic))
      fprintf(file, "@PLT");
    return;
  }
 ...
 through non-vector mode.  */
    if ((code == 'v' || code == 'V') || VECTOR_NOFP_REG_P(x) || (FP_REG_P(x) && VECTOR_MODE_P(GET_MODE(x))) || (VECTOR_REG_P(x) && (GET_MODE_SIZE(GET_MODE(x)) / s390_class_max_nregs(FP_REGS, GET_MODE(x))) > 8))
...
{
  if (size == 8 && aligned_p && GET_CODE(x) == CONST_INT && INTVAL(x) < INT_MIN)
  {
...
reg_used_in_mem_p(int regno, rtx x)
{
  ...
  else if (code == SET && GET_CODE(SET_DEST(x)) == PC)
  {
  ...
  {
    if (fmt[i] == 'e' && reg_used_in_mem_p(regno, XEXP(x, i)))
      return true;
...
int s390_agen_dep_p(rtx_insn *dep_insn, rtx_insn *insn)
{
  rtx dep_rtx = PATTERN(dep_insn);
  int i;

  if (GET_CODE(dep_rtx) == SET && addr_generation_dependency_p(dep_rtx, insn))
    return 1;
...
annotate_constant_pool_refs_1(rtx *x)
{
  ...
  gcc_assert(GET_CODE(*x) != SYMBOL_REF || !CONSTANT_POOL_ADDRESS_P(*x));
  ...
  if (GET_CODE(*x) == MEM)
  {
    ...
    if (GET_CODE(memref) == SYMBOL_REF && CONSTANT_POOL_ADDRESS_P(memref))
    {
    ...
    if (GET_CODE(memref) == CONST && GET_CODE(XEXP(memref, 0)) == PLUS && GET_CODE(XEXP(XEXP(memref, 0), 1)) == CONST_INT && GET_CODE(XEXP(XEXP(memref, 0), 0)) == SYMBOL_REF && CONSTANT_POOL_ADDRESS_P(XEXP(XEXP(memref, 0), 0)))
    {
  ...
  if (GET_CODE(*x) == SET)
  {
    ...
    if (GET_CODE(addrref) == SYMBOL_REF && CONSTANT_POOL_ADDRESS_P(addrref))
    {
      rtx base = cfun->machine->base_reg;
    ...
    if (GET_CODE(addrref) == CONST && GET_CODE(XEXP(addrref, 0)) == PLUS && GET_CODE(XEXP(XEXP(addrref, 0), 1)) == CONST_INT && GET_CODE(XEXP(XEXP(addrref, 0), 0)) == SYMBOL_REF && CONSTANT_POOL_ADDRESS_P(XEXP(XEXP(addrref, 0), 0)))
    {
...
find_constant_pool_ref_1(rtx x, rtx *ref)
{
  ...
  if (GET_CODE(x) == UNSPEC_VOLATILE && XINT(x, 1) == UNSPECV_POOL_ENTRY)
    return;
  ...
  if (GET_CODE(x) == UNSPEC && XINT(x, 1) == UNSPEC_LTREF)
  {
    ...
    gcc_assert(GET_CODE(sym) == SYMBOL_REF && CONSTANT_POOL_ADDRESS_P(sym));
...
replace_constant_pool_ref_1(rtx *x, rtx ref, rtx offset)
{
  ...
  if (GET_CODE(*x) == UNSPEC && XINT(*x, 1) == UNSPEC_LTREF && XVECEXP(*x, 0, 0) == ref)
  {
    *x = gen_rtx_PLUS(Pmode, XVECEXP(*x, 0, 1), offset);
    return;
  }

  if (GET_CODE(*x) == PLUS && GET_CODE(XEXP(*x, 1)) == CONST_INT && GET_CODE(XEXP(*x, 0)) == UNSPEC && XINT(XEXP(*x, 0), 1) == UNSPEC_LTREF && XVECEXP(XEXP(*x, 0), 0, 0) == ref)
  {
...
{
  if (INSN_P(insn) && GET_CODE(PATTERN(insn)) == PARALLEL && GET_CODE(XVECEXP(PATTERN(insn), 0, 0)) == UNSPEC && (XINT(XVECEXP(PATTERN(insn), 0, 0), 1) == UNSPEC_EXECUTE || XINT(XVECEXP(PATTERN(insn), 0, 0), 1) == UNSPEC_EXECUTE_JUMP))
  {
...
s390_dump_pool(struct constant_pool *pool, bool remote_label)
{
  ...
  for (i = 0; i < NR_C_MODES; i++)
    for (c = pool->constants[i]; c; c = c->next)
    {
      ...
      if (GET_CODE(value) == CONST && GET_CODE(XEXP(value, 0)) == UNSPEC && XINT(XEXP(value, 0), 1) == UNSPEC_LTREL_OFFSET && XVECLEN(XEXP(value, 0), 0) == 1)
...
s390_mainpool_start(void)
{
  ...
  {
    if (NONJUMP_INSN_P(insn) && GET_CODE(PATTERN(insn)) == SET && GET_CODE(SET_SRC(PATTERN(insn))) == UNSPEC_VOLATILE && XINT(SET_SRC(PATTERN(insn)), 1) == UNSPECV_MAIN_POOL)
    {
 ...
 function body within the text section.  */
    if (NOTE_P(insn) && NOTE_KIND(insn) == NOTE_INSN_SWITCH_TEXT_SECTIONS && !pool->emit_pool_after)
...
s390_chunkify_start(void)
{
  ...
  for (insn = get_insns(); insn; insn = NEXT_INSN(insn))
  {
    ...
    if (!curr_pool || INSN_ADDRESSES_SIZE() <= (size_t)INSN_UID(insn) || INSN_ADDRESSES(INSN_UID(insn)) == -1)
      continue;
 ...
 a jump table.  */
    ...
    if (LABEL_P(insn) && (LABEL_PRESERVE_P(insn) || LABEL_NAME(insn)))
    {
    ...
    else if (tablejump_p(insn, NULL, &table))
    {
      ...
      for (i = 0; i < XVECLEN(vec_pat, diff_p); i++)
      {
        ...
        if (s390_find_pool(pool_list, label) != s390_find_pool(pool_list, insn))
 ...
 check all potential targets.  */
  ...
  for (insn = get_insns(); insn; insn = NEXT_INSN(insn))
    if (LABEL_P(insn) && bitmap_bit_p(far_labels, CODE_LABEL_NUMBER(insn)))
    {
...
void s390_output_pool_entry(rtx exp, machine_mode mode, unsigned int align)
{
...
bool s390_const_int_pool_entry_p(rtx mem, HOST_WIDE_INT *val)
{
...
rtx s390_return_addr_rtx(int count, rtx frame ATTRIBUTE_UNUSED)
{
  int offset;
  rtx addr;
...
s390_regs_ever_clobbered(char regs_ever_clobbered[])
{
  ...
  if (crtl->calls_eh_return || cfun->machine->has_landing_pad_p)
    for (i = 0; EH_RETURN_DATA_REGNO(i) != INVALID_REGNUM; i++)
      if (crtl->calls_eh_return || (cfun->machine->has_landing_pad_p && df_regs_ever_live_p(EH_RETURN_DATA_REGNO(i))))
  ...
  FOR_EACH_BB_FN(cur_bb, cfun)
  {
    FOR_BB_INSNS(cur_bb, cur_insn)
    {
      ...
      {
        if (GET_CODE(pat) == SET && GENERAL_REG_P(SET_DEST(pat)))
        {
          /* lgdr  */
          if (GET_MODE(SET_SRC(pat)) == DImode && FP_REG_P(SET_SRC(pat)))
            continue;
        ...
        /* lm / lmg */
        if (GET_CODE(pat) == PARALLEL && load_multiple_operation(pat, VOIDmode))
          continue;
      }
...
s390_frame_area(int *area_bottom, int *area_top)
{
  ...
  {
    b = (cfun_frame_layout.gprs_offset + cfun_frame_layout.first_restore_gpr * UNITS_PER_LONG);
    t = b + (cfun_frame_layout.last_restore_gpr - cfun_frame_layout.first_restore_gpr + 1) * UNITS_PER_LONG;
  }

  if (TARGET_64BIT && cfun_save_high_fprs_p)
  {
    b = MIN(b, cfun_frame_layout.f8_offset);
    t = MAX(t, (cfun_frame_layout.f8_offset + cfun_frame_layout.high_fprs * 8));
  }
...
s390_register_info_gprtofpr()
{
 ...
 GPR save slot.  */
    while ((!call_used_regs[save_reg_slot] || df_regs_ever_live_p(save_reg_slot) || cfun_fpr_save_p(save_reg_slot)) && FP_REGNO_P(save_reg_slot))
      save_reg_slot++;
...
s390_register_info_stdarg_fpr()
{
  ...
  /* Save the FP argument regs for stdarg. f0, f2 for 31 bit and
     f0-f4 for 64 bit.  */
  if (!cfun->stdarg || !TARGET_HARD_FLOAT || !cfun->va_list_fpr_size || crtl->args.info.fprs >= FP_ARG_NUM_REG)
    return;
...
s390_register_info_stdarg_gpr()
{
  ...
  if (!cfun->stdarg || !cfun->va_list_gpr_size || crtl->args.info.gprs >= GP_ARG_NUM_REG)
    return;
  ...
  if (FP_REGNO_P(cfun_gpr_save_slot(GPR6_REGNUM)) && min_gpr <= GPR6_REGNUM && max_gpr >= GPR6_REGNUM)
  {
    if (cfun_frame_layout.first_restore_gpr == -1 || cfun_frame_layout.first_restore_gpr > GPR6_REGNUM)
      cfun_frame_layout.first_restore_gpr = GPR6_REGNUM;
    if (cfun_frame_layout.last_restore_gpr == -1 || cfun_frame_layout.last_restore_gpr < GPR6_REGNUM)
      cfun_frame_layout.last_restore_gpr = GPR6_REGNUM;
  }

  if (cfun_frame_layout.first_save_gpr == -1 || cfun_frame_layout.first_save_gpr > min_gpr)
    cfun_frame_layout.first_save_gpr = min_gpr;

  if (cfun_frame_layout.last_save_gpr == -1 || cfun_frame_layout.last_save_gpr < max_gpr)
...
s390_register_info()
{
  ...
  clobbered_regs[12] |= ((flag_pic && df_regs_ever_live_p(PIC_OFFSET_TABLE_REGNUM)) || (flag_split_stack && cfun->stdarg && (crtl->is_leaf || TARGET_TPF_PROFILING || has_hard_reg_initial_val(Pmode, RETURN_REGNUM))));

  clobbered_regs[BASE_REGNUM] |= (cfun->machine->base_reg && REGNO(cfun->machine->base_reg) == BASE_REGNUM);

  clobbered_regs[HARD_FRAME_POINTER_REGNUM] |= !!frame_pointer_needed;
  ...
  /* On pre z900 machines this might take until machine dependent
     ...
     slot.  */
  clobbered_regs[RETURN_REGNUM] |= (!crtl->is_leaf || TARGET_TPF_PROFILING || cfun_frame_layout.save_return_addr_p || crtl->calls_eh_return);

  clobbered_regs[STACK_POINTER_REGNUM] |= (!crtl->is_leaf || TARGET_TPF_PROFILING || cfun_save_high_fprs_p || get_frame_size() > 0 || (reload_completed && cfun_frame_layout.frame_size > 0) || cfun->calls_alloca);
...
global_not_special_regno_p(int regno)
{
  return (global_regs[regno]
          ...
          && !(regno == STACK_POINTER_REGNUM || regno == RETURN_REGNUM || regno == BASE_REGNUM || (flag_pic && regno == (int)PIC_OFFSET_TABLE_REGNUM)));
}
...
s390_optimize_register_info()
{
  ...
  clobbered_regs[RETURN_REGNUM] |= (TARGET_TPF_PROFILING
                                    /* When expanding builtin_return_addr in ESA mode we do not
                                       ...
                                       clobbered.  */
                                    || cfun_frame_layout.save_return_addr_p || crtl->calls_eh_return);
...
s390_frame_info(void)
{
  ...
  if (!TARGET_PACKED_STACK)
  {
    ...
    cfun_frame_layout.gprs_offset = (cfun_frame_layout.first_save_gpr_slot * UNITS_PER_LONG);
  }
  else if (TARGET_BACKCHAIN)
  {
    /* Kernel stack layout - packed stack, backchain, no float  */
    gcc_assert(TARGET_SOFT_FLOAT);
    cfun_frame_layout.backchain_offset = (STACK_POINTER_OFFSET - UNITS_PER_LONG);
 ...
 stack pointer which resides in between.  */
    ...
    cfun_frame_layout.gprs_offset = cfun_frame_layout.backchain_offset - cfun_gprs_save_area_size;
 ...
 keep area calculations valid.  */
  ...
  else
  {
    ...
    num_fprs = (cfun->stdarg ? 2
                             : (cfun_fpr_save_p(FPR0_REGNUM) + cfun_fpr_save_p(FPR2_REGNUM)));
    cfun_frame_layout.f0_offset = cfun_frame_layout.f4_offset - 8 * num_fprs;

    cfun_frame_layout.gprs_offset = cfun_frame_layout.f0_offset - cfun_gprs_save_area_size;

    cfun_frame_layout.f8_offset = (cfun_frame_layout.gprs_offset - cfun_frame_layout.high_fprs * 8);
  }
  ...
  if (crtl->is_leaf && !TARGET_TPF_PROFILING && cfun_frame_layout.frame_size == 0 && !cfun->calls_alloca)
    return;
...
s390_init_frame_layout(void)
{
  ...
  /* Skip the frame layout if we're naked */
  ...
  do
  {
    ...
    base_used = crtl->uses_const_pool || (!DISP_IN_RANGE(frame_size) && !CONST_OK_FOR_K(frame_size));
...
s390_optimize_nonescaping_tx(void)
{
  ...
  for (bb_index = 0; bb_index < n_basic_blocks_for_fn(cfun); bb_index++)
  {
    ...
    FOR_BB_INSNS(bb, insn)
    {
      ...
      if (GET_CODE(pat) != SET || GET_CODE(SET_SRC(pat)) != UNSPEC_VOLATILE)
        continue;
      ...
      if (XINT(SET_SRC(pat), 1) == UNSPECV_TBEGIN)
      {
             ...
             tmp = NEXT_INSN(tmp))
        {
          ...
          if (!REG_P(cc) || !CC_REGNO_P(REGNO(cc)) || GET_MODE(cc) != CCRAWmode || GET_CODE(XEXP(XEXP(ite, 0), 1)) != CONST_INT)
            return;
...
s390_hard_regno_mode_ok(unsigned int regno, machine_mode mode)
{
  ...
  {
  case VEC_REGS:
    return ((GET_MODE_CLASS(mode) == MODE_INT && s390_class_max_nregs(VEC_REGS, mode) == 1) || mode == DFmode || (TARGET_VXE && mode == SFmode) || s390_vector_mode_supported_p(mode));
    break;
  case FP_REGS:
    if (TARGET_VX && ((GET_MODE_CLASS(mode) == MODE_INT && s390_class_max_nregs(FP_REGS, mode) == 1) || mode == DFmode || s390_vector_mode_supported_p(mode)))
      return true;
  ...
  case GENERAL_REGS:
    if (REGNO_PAIR_OK(regno, mode))
    {
      if (TARGET_ZARCH || (mode != TFmode && mode != TCmode && mode != TDmode))
        return true;
    }
    break;
  case CC_REGS:
...
{
  return ((mode1 == SFmode || mode1 == DFmode) == (mode2 == SFmode || mode2 == DFmode));
}

/* Return nonzero if register OLD_REG can be renamed to register NEW_REG.  */

bool s390_hard_regno_rename_ok(unsigned int old_reg, unsigned int new_reg)
{
  /* Once we've decided upon a register to use as base register, it must
     no longer be used for any other purpose.  */
  if (cfun->machine->base_reg)
    if (REGNO(cfun->machine->base_reg) == old_reg || REGNO(cfun->machine->base_reg) == new_reg)
      return false;
  ...
  if (GENERAL_REGNO_P(new_reg) && !call_used_regs[new_reg] && cfun_gpr_save_slot(new_reg) == SAVE_SLOT_NONE)
    return false;
...
s390_hard_regno_scratch_ok(unsigned int regno)
{
  ...
  if (GENERAL_REGNO_P(regno) && !call_used_regs[regno] && cfun_gpr_save_slot(regno) == SAVE_SLOT_NONE)
    return false;
                                    ...
                                    machine_mode mode)
{
  ...
  if (!TARGET_64BIT && TARGET_ZARCH && GET_MODE_SIZE(mode) > 4 && (!flag_pic || regno != PIC_OFFSET_TABLE_REGNUM) && ((regno >= 6 && regno <= 15) || regno == 32))
    return true;

  if (TARGET_VX && GET_MODE_SIZE(mode) > 8 && (((TARGET_64BIT && regno >= 24 && regno <= 31)) || (!TARGET_64BIT && (regno == 18 || regno == 19))))
    return true;
...
int s390_class_max_nregs(enum reg_class rclass, machine_mode mode)
{
  int reg_size;
 ...
 full VRs.  */
    if (TARGET_VX && SCALAR_FLOAT_MODE_P(mode) && GET_MODE_SIZE(mode) >= 16 && !(TARGET_VXE && mode == TFmode))
                           ...
                           reg_class_t rclass)
{
  ...
  /* 128-bit values have different representations in floating point and
     vector registers.  */
  if (reg_classes_intersect_p(VEC_REGS, rclass) && ((s390_is_fpr128(from_mode) && s390_is_vr128(to_mode)) || (s390_is_vr128(from_mode) && s390_is_fpr128(to_mode))))
    return false;
  ...
  if (reg_classes_intersect_p(VEC_REGS, rclass) && (GET_MODE_SIZE(small_mode) < 8 || s390_class_max_nregs(VEC_REGS, big_mode) == 1))
    return false;
...
s390_can_eliminate(const int from, const int to)
{
  ...
  gcc_assert(to == STACK_POINTER_REGNUM || to == HARD_FRAME_POINTER_REGNUM);

  gcc_assert(from == FRAME_POINTER_REGNUM || from == ARG_POINTER_REGNUM || from == RETURN_ADDRESS_POINTER_REGNUM);

  /* Make sure we actually saved the return address.  */
  if (from == RETURN_ADDRESS_POINTER_REGNUM)
    if (!crtl->calls_eh_return && !cfun->stdarg && !cfun_frame_layout.save_return_addr_p)
      return false;
...
s390_initial_elimination_offset(int from, int to)
{
  ...
  case FRAME_POINTER_REGNUM:
    offset = (get_frame_size() + STACK_POINTER_OFFSET + crtl->outgoing_args_size);
    break;
...
save_gprs(rtx base, int offset, int first, int last)
{
  ...
  if (first >= 6 && !global_not_special_regno_p(first))
  {
    ...
    for (i = 0; i < XVECLEN(pat, 0); i++)
      if (GET_CODE(XVECEXP(pat, 0, i)) == SET && !global_not_special_regno_p(REGNO(SET_SRC(XVECEXP(pat,
                                                                                                   0, i)))))
  ...
  else if (last >= 6)
  {
    ...
    for (i = 0; i < XVECLEN(note, 0); i++)
      if (GET_CODE(XVECEXP(note, 0, i)) == SET && !global_not_special_regno_p(REGNO(SET_SRC(XVECEXP(note,
                                                                                                    0, i)))))
...
namespace
{
  ...
  pass_s390_early_mach::execute(function *fun)
  {
    ...
    /* Skip the prologue if we're naked */
    if (has_func_attr (NULL_TREE, "naked"))
      return 0;
...
s390_prologue_plus_offset(rtx target, rtx reg, rtx offset, bool frame_related_p)
{
  ...
  else
  {
    if (!satisfies_constraint_K(offset)                               /* ahi/aghi */
        && (!TARGET_EXTIMM || (!satisfies_constraint_Op(offset)       /* alfi/algfi */
                     ...
                     rtx temp_reg)
{
  ...
  HOST_WIDE_INT probe_interval = 1 << param_stack_clash_protection_probe_interval;
  HOST_WIDE_INT guard_size = 1 << param_stack_clash_protection_guard_size;
    ...
    else
    {
     ...
     emit it to set the CFA reg back to r15.  */
      ...
      if (last_probe_offset >= probe_interval)
        s390_emit_stack_probe(gen_rtx_PLUS(Pmode,
                                           stack_pointer_rtx,
                                           GEN_INT(residual - UNITS_PER_LONG)));
...
const char *
s390_emit_branch_indirect(void)
{
  /* Skip the epilogue if we're naked */
  if (has_func_attr (NULL_TREE, "naked"))
    return "";
  else
    return "br\t%%r14";
...
void s390_emit_prologue(void)
{
  ...
  rtx temp_reg;
  if (!has_hard_reg_initial_val(Pmode, RETURN_REGNUM) && !crtl->is_leaf && !TARGET_TPF_PROFILING)
  ...
  HOST_WIDE_INT probe_interval = 1 << param_stack_clash_protection_probe_interval;
  HOST_WIDE_INT last_probe_offset = (TARGET_BACKCHAIN
  ...
  if (cfun_frame_layout.first_save_gpr != -1)
  {
    insn = save_gprs(stack_pointer_rtx,
                     cfun_frame_layout.gprs_offset +
                         UNITS_PER_LONG * (cfun_frame_layout.first_save_gpr - cfun_frame_layout.first_save_gpr_slot),
 ...
 then LAST_PROBE_OFFSET can move even closer to sp.  */
    last_probe_offset = (cfun_frame_layout.gprs_offset +
                         UNITS_PER_LONG * (cfun_frame_layout.first_save_gpr - cfun_frame_layout.first_save_gpr_slot));
  ...
  int offset = cfun_frame_layout.f0_offset;

  /* Save f0 and f2.  */
  for (int i = FPR0_REGNUM; i <= FPR0_REGNUM + 1; i++)
  {
  ...
  for (int i = FPR4_REGNUM; i <= FPR4_REGNUM + 1; i++)
  {
    if (cfun_fpr_save_p(i))
    {
  ...
  if (TARGET_PACKED_STACK && cfun_save_high_fprs_p && cfun_frame_layout.f8_offset + cfun_frame_layout.high_fprs * 8 > 0)
  {
    offset = (cfun_frame_layout.f8_offset + (cfun_frame_layout.high_fprs - 1) * 8);
  ...
  if (cfun_frame_layout.frame_size > 0)
  {
    ...
    if (s390_stack_size)
    {
     ...
     not match the test under mask pattern.  */
        ...
        {
          HOST_WIDE_INT stack_check_mask = ((s390_stack_size - 1) & ~(stack_guard - 1));
    ...
    if (s390_warn_framesize > 0 && cfun_frame_layout.frame_size >= s390_warn_framesize)
 ...
 moved below the use of the stack slots.  */
    ...
    for (int i = FPR8_REGNUM; i <= next_fpr; i++)
      if (cfun_fpr_save_p(i))
      {
        rtx addr = plus_constant(Pmode, stack_pointer_rtx,
                                 cfun_frame_layout.frame_size + cfun_frame_layout.f8_offset + offset);
...
void s390_emit_epilogue(bool sibcall)
{
 ...
 profiling mechanisms.  */
  ...
  else if (DISP_IN_RANGE(cfun_frame_layout.frame_size + area_bottom) && DISP_IN_RANGE(cfun_frame_layout.frame_size + area_top - 1))
  {
    /* Area is in range.  */
         ...
         i++)
    {
      if (global_not_special_regno_p(i))
      {
        addr = plus_constant(Pmode, frame_pointer,
                             offset + cfun_frame_layout.gprs_offset + (i - cfun_frame_layout.first_save_gpr_slot) * UNITS_PER_LONG);
        addr = gen_rtx_MEM(Pmode, addr);
        set_mem_alias_set(addr, get_frame_alias_set());
 ...
 This optimization is only helpful on in-order machines.  */
    if (!sibcall && cfun_gpr_save_slot(RETURN_REGNUM) == SAVE_SLOT_STACK && s390_tune <= PROCESSOR_2097_Z10)
    {
      int return_regnum = find_unused_clobbered_reg();
      if (!return_regnum || (TARGET_INDIRECT_BRANCH_NOBP_RET_OPTION && !TARGET_CPU_Z10 && return_regnum == INDIRECT_BRANCH_THUNK_REGNUM))
      {
      ...
      addr = plus_constant(Pmode, frame_pointer,
                           offset + cfun_frame_layout.gprs_offset + (RETURN_REGNUM - cfun_frame_layout.first_save_gpr_slot) * UNITS_PER_LONG);
      addr = gen_rtx_MEM(Pmode, addr);
      set_mem_alias_set(addr, get_frame_alias_set());
    ...
    insn = restore_gprs(frame_pointer,
                        offset + cfun_frame_layout.gprs_offset + (cfun_frame_layout.first_restore_gpr - cfun_frame_layout.first_save_gpr_slot) * UNITS_PER_LONG,
...
{
  if (cfun->machine->base_reg && !call_used_regs[REGNO(cfun->machine->base_reg)])
...
void s390_output_split_stack_data(rtx parm_block, rtx call_done,
                                  rtx frame_size, rtx args_size)
{
  rtx ops[] = {parm_block, call_done};

  switch_to_section(targetm.asm_out.function_rodata_section(current_function_decl, false));
...
void s390_expand_split_stack_prologue(void)
{
  ...
  if (morestack_ref == NULL_RTX)
  {
    ...
    SYMBOL_REF_FLAGS(morestack_ref) |= (SYMBOL_FLAG_LOCAL | SYMBOL_FLAG_FUNCTION);
  }
...
bool s390_can_use_simple_return_insn(void)
{
  return true;
}
...
bool s390_can_use_return_insn(void)
{
  int i;
  ...
  if (!TARGET_64BIT && (cfun_fpr_save_p(FPR4_REGNUM) || cfun_fpr_save_p(FPR6_REGNUM)))
    return false;
...
{
  return ((TARGET_VX_ABI && typelist == 0 && VECTOR_TYPE_P(TREE_TYPE(val)) && (funcdecl == NULL_TREE || (TREE_CODE(funcdecl) == FUNCTION_DECL && DECL_BUILT_IN_CLASS(funcdecl) != BUILT_IN_MD)))
...
s390_function_arg_integer(machine_mode mode, const_tree type)
{
  ...
  if (AGGREGATE_TYPE_P(type) && exact_log2(size) >= 0 && !s390_function_arg_float(mode, type))
    return true;
 ...
 stack.  */
  ...
  else if (s390_function_arg_integer(arg.mode, arg.type))
  {
    ...
    else if (n_gprs == 2)
    {
      ...
      RTVEC_ELT(p, 0) = gen_rtx_EXPR_LIST(SImode, gen_rtx_REG(SImode, cum->gprs + 2),
                                          const0_rtx);
      RTVEC_ELT(p, 1) = gen_rtx_EXPR_LIST(SImode, gen_rtx_REG(SImode, cum->gprs + 3),
                                          GEN_INT(4));
...
{
  if (INTEGRAL_MODE_P(mode) && GET_MODE_SIZE(mode) < UNITS_PER_LONG)
  {
...
s390_va_start(tree valist, rtx nextarg ATTRIBUTE_UNUSED)
{
  ...
  if (flag_split_stack && (lookup_attribute("no_split_stack", DECL_ATTRIBUTES(cfun->decl)) == NULL) && cfun->machine->split_stack_varargs_pointer == NULL_RTX)
  {
    rtx reg;
    rtx_insn *seq;
  ...
  /* Find the overflow area.
     ...
     pointer.  */
  if (n_gpr + cfun->va_list_gpr_size > GP_ARG_NUM_REG || n_fpr + cfun->va_list_fpr_size > FP_ARG_NUM_REG || TARGET_VX_ABI)
  {
    if (cfun->machine->split_stack_varargs_pointer == NULL_RTX)
  ...
  if ((cfun->va_list_gpr_size && n_gpr < GP_ARG_NUM_REG) || (cfun->va_list_fpr_size && n_fpr < FP_ARG_NUM_REG))
  {
...
void s390_expand_tbegin(rtx dest, rtx tdb, rtx retry, bool clobber_fprs_p)
{
...
void s390_function_profiler(FILE *file, int labelno ATTRIBUTE_UNUSED)
{
  rtx op[4];
...
s390_encode_section_info(tree decl, rtx rtl, int first)
{
 ...
 that can go wrong (i.e. no FUNC_DECLs).  */
  ...
  if (MEM_P(rtl) && GET_CODE(XEXP(rtl, 0)) == SYMBOL_REF && TREE_CONSTANT_POOL_ADDRESS_P(XEXP(rtl, 0)))
  {
                     ...
                     tree function)
{
  ...
  if (TARGET_64BIT)
  {
    ...
    if ((!DISP_IN_RANGE(delta) && !CONST_OK_FOR_K(delta) && !CONST_OK_FOR_Os(delta)) || (!DISP_IN_RANGE(vcall_offset) && !CONST_OK_FOR_K(vcall_offset) && !CONST_OK_FOR_Os(vcall_offset)))
    {
      op[5] = gen_label_rtx();
  ...
  else
  {
    ...
    if (!vcall_offset || (!DISP_IN_RANGE(delta) && !CONST_OK_FOR_K(delta) && !CONST_OK_FOR_Os(delta)) || (!DISP_IN_RANGE(delta) && !CONST_OK_FOR_K(vcall_offset) && !CONST_OK_FOR_Os(vcall_offset)))
    {
      op[5] = gen_label_rtx();
...
void s390_indirect_branch_via_thunk(unsigned int regno,
                                    ...
                                    enum s390_indirect_branch_type type)
{
  ...
  if ((option == s390_opt_indirect_branch_jump && cfun->machine->indirect_branch_jump == indirect_branch_thunk) || (option == s390_opt_indirect_branch_call && cfun->machine->indirect_branch_call == indirect_branch_thunk) || (option == s390_opt_function_return_reg && cfun->machine->function_return_reg == indirect_branch_thunk) || (option == s390_opt_function_return_mem && cfun->machine->function_return_mem == indirect_branch_thunk))
  {
    if (TARGET_CPU_Z10)
...
void s390_indirect_branch_via_inline_thunk(rtx execute_target)
{
  if (TARGET_INDIRECT_BRANCH_TABLE)
  {
    char label[32];
...
s390_function_ok_for_sibcall(tree decl, tree exp)
{
  ...
  /* The thunks for indirect branches require r1 if no exrl is
     ...
     call.  */
  if (TARGET_INDIRECT_BRANCH_NOBP_CALL && !TARGET_CPU_Z10 && !decl)
    return false;
               ...
               rtx retaddr_reg)
{
 ...
 replace the symbol itself with the PLT stub.  */
  ...
  /* If it is already an indirect call or the code above moved the
     ...
     register 1.  */
  if (retaddr_reg == NULL_RTX && GET_CODE(addr_location) != SYMBOL_REF && !plt31_call_p)
  {
    emit_move_insn(gen_rtx_REG(Pmode, SIBCALL_REGNUM), addr_location);
    addr_location = gen_rtx_REG(Pmode, SIBCALL_REGNUM);
  }

  if (TARGET_INDIRECT_BRANCH_NOBP_CALL && GET_CODE(addr_location) != SYMBOL_REF && !plt31_call_p)
  {
...
void s390_emit_tpf_eh_return(rtx target)
{
  rtx_insn *insn;
  rtx reg, orig_ra;
...
s390_optimize_prologue(void)
{
  ...
  /* Skip the optimization if we're naked */
  if (has_func_attr (NULL_TREE, "naked"))
    return;
 ...
 GPRs if possible.  */
    if (TARGET_Z10)
    {
      ...
      if (GET_CODE(tmp_pat) == SET && GET_MODE(SET_SRC(tmp_pat)) == DImode && REG_P(SET_SRC(tmp_pat)) && REG_P(SET_DEST(tmp_pat)))
      {
        ...
        if (!((GENERAL_REGNO_P(src_regno) && FP_REGNO_P(dest_regno)) || (FP_REGNO_P(src_regno) && GENERAL_REGNO_P(dest_regno))))
          continue;
        ...
        if (!call_used_regs[fpr_regno] || call_used_regs[gpr_regno])
          continue;
    ...
    if (GET_CODE(pat) == PARALLEL && store_multiple_operation(pat, VOIDmode))
    {
      set = XVECEXP(pat, 0, 0);
      ...
      if (GET_CODE(base) != REG || off < 0)
        continue;
      if (cfun_frame_layout.first_save_gpr != -1 && (cfun_frame_layout.first_save_gpr < first || cfun_frame_layout.last_save_gpr > last))
        continue;
      if (REGNO(base) != STACK_POINTER_REGNUM && REGNO(base) != HARD_FRAME_POINTER_REGNUM)
        continue;
      ...
      if (cfun_frame_layout.first_save_gpr != -1)
      {
        rtx s_pat = save_gprs(base,
                              off + (cfun_frame_layout.first_save_gpr - first) * UNITS_PER_LONG,
                              cfun_frame_layout.first_save_gpr,
                              cfun_frame_layout.last_save_gpr);
    ...
    if (cfun_frame_layout.first_save_gpr == -1 && GET_CODE(pat) == SET && GENERAL_REG_P(SET_SRC(pat)) && GET_CODE(SET_DEST(pat)) == MEM)
    {
      set = pat;
      first = REGNO(SET_SRC(set));
      offset = const0_rtx;
    ...
    if (GET_CODE(pat) == PARALLEL && load_multiple_operation(pat, VOIDmode))
    {
      ...
      if (cfun_frame_layout.first_restore_gpr != -1 && (cfun_frame_layout.first_restore_gpr < first || cfun_frame_layout.last_restore_gpr > last))
        continue;
      if (REGNO(base) != STACK_POINTER_REGNUM && REGNO(base) != HARD_FRAME_POINTER_REGNUM)
        continue;
      if (first > BASE_REGNUM || last < BASE_REGNUM)
      ...
      if (cfun_frame_layout.first_restore_gpr != -1)
      {
        rtx rpat = restore_gprs(base,
                                off + (cfun_frame_layout.first_restore_gpr - first) * UNITS_PER_LONG,
                                cfun_frame_layout.first_restore_gpr,
                                cfun_frame_layout.last_restore_gpr);
     ...
     longer need to save.  */
        ...
        for (rtx *ptr = &REG_NOTES(rpat); *ptr;)
          if (REG_NOTE_KIND(*ptr) == REG_CFA_RESTORE && ((int)REGNO(XEXP(*ptr, 0)) < cfun_frame_layout.first_restore_gpr))
            *ptr = XEXP(*ptr, 1);
          else
            ptr = &XEXP(*ptr, 1);
    ...
    if (cfun_frame_layout.first_restore_gpr == -1 && GET_CODE(pat) == SET && GENERAL_REG_P(SET_DEST(pat)) && GET_CODE(SET_SRC(pat)) == MEM)
    {
      set = pat;
      first = REGNO(SET_DEST(set));
      offset = const0_rtx;
...
s390_fix_long_loop_prediction(rtx_insn *insn)
{
  ...
  if (GET_CODE(PATTERN(insn)) == PARALLEL || !set || SET_DEST(set) != pc_rtx || GET_CODE(SET_SRC(set)) != IF_THEN_ELSE)
    return false;

  /* Skip conditional returns.  */
  if (ANY_RETURN_P(XEXP(SET_SRC(set), 1)) && XEXP(SET_SRC(set), 2) == pc_rtx)
    return false;

  label_ref = (GET_CODE(XEXP(SET_SRC(set), 1)) == LABEL_REF ? XEXP(SET_SRC(set), 1) : XEXP(SET_SRC(set), 2));

  gcc_assert(GET_CODE(label_ref) == LABEL_REF);

  code_label = XEXP(label_ref, 0);

  if (INSN_ADDRESSES(INSN_UID(code_label)) == -1 || INSN_ADDRESSES(INSN_UID(insn)) == -1 || (INSN_ADDRESSES(INSN_UID(insn)) - INSN_ADDRESSES(INSN_UID(code_label)) < PREDICT_DISTANCE))
    return false;
...
{
  return reg_referenced_p(reg, PATTERN(insn)) && !reg_used_in_mem_p(REGNO(reg), PATTERN(insn));
}
...
s390_z10_optimize_cmp(rtx_insn *insn)
{
 ...
 instructions.  */
    ...
    if (!pattern || SET_DEST(pattern) != pc_rtx || GET_CODE(SET_SRC(pattern)) != IF_THEN_ELSE)
      return false;
  ...
  else if (GET_CODE(PATTERN(insn)) == SET)
  {
    ...
    if (!REG_P(dest) || !CC_REGNO_P(REGNO(dest)) || GET_CODE(src) != COMPARE)
      return false;
 ...
 jump when passing NULL_RTX as condition.  */
  ...
  if (prev_insn != NULL_RTX && INSN_P(prev_insn) && reg_referenced_p(*op1, PATTERN(prev_insn)))
  ...
  if (next_insn != NULL_RTX && INSN_P(next_insn) && s390_non_addr_reg_read_p(*op1, next_insn))
  {
    if (prev_insn != NULL_RTX && INSN_P(prev_insn) && s390_non_addr_reg_read_p(*op0, prev_insn))
    {
      if (REGNO(*op1) == 0)
...

{
  ...
  FOR_BB_INSNS_REVERSE(loop->latch, insn)
  {
    ...
    if (MEM_P(SET_DEST(set)) && s390_decompose_address(XEXP(SET_DEST(set), 0), &addr_store))
    {
      store_insn = insn;
      break;
    }
 ...
 anyway.  */
    if (MEM_P(SET_DEST(set)) && s390_decompose_address(XEXP(SET_DEST(set), 0), NULL))
      return false;

    FOR_EACH_SUBRTX(iter, array, SET_SRC(set), NONCONST)
    if (MEM_P(*iter) && s390_decompose_address(XEXP(*iter, 0), &addr_load) && rtx_equal_p(addr_load.base, addr_store.base) && rtx_equal_p(addr_load.indx, addr_store.indx) && rtx_equal_p(addr_load.disp, addr_store.disp))
    {
      if ((addr_load.base != NULL_RTX && TEST_HARD_REG_BIT(modregs, REGNO(addr_load.base))) || (addr_load.indx != NULL_RTX && TEST_HARD_REG_BIT(modregs, REGNO(addr_load.indx))))
        return true;
    }
  }
  return false;
...
s390_adjust_loops()
{
  ...
  for (auto loop : loops_list(cfun, LI_ONLY_INNERMOST))
  {
    ...
    if (loop->latch == NULL || pc_set(BB_END(loop->latch)) == NULL_RTX || !s390_adjust_loop_scan_osc(loop))
    {
      if (dump_file)
      {
        if (loop->latch == NULL)
...
s390_reorg(void)
{
 ...
 following manipulations.  */
    ...
    for (insn = get_insns(); insn; insn = NEXT_INSN(insn))
    {
      ...
      if ((GET_CODE(PATTERN(insn)) == PARALLEL || GET_CODE(PATTERN(insn)) == SET) && s390_tune == PROCESSOR_2097_Z10)
...
s390_z10_prevent_earlyload_conflicts(rtx_insn **ready, int *nready_p)
{
  ...
  if (set == NULL_RTX || !REG_P(SET_DEST(set)) || GET_MODE_CLASS(GET_MODE(SET_DEST(set))) != MODE_FLOAT)
    return;
...
s390_bb_fallthru_entry_likely(basic_block bb)
{
  ...
  if (e != fallthru_edge && e->probability >= profile_probability::likely())
    return false;
...
s390_is_fpd(rtx_insn *insn)
{
  ...
  return get_attr_z13_unit_fpd(insn) || get_attr_z14_unit_fpd(insn) || get_attr_z15_unit_fpd(insn) || get_attr_z16_unit_fpd(insn);
}
...
s390_is_fxd(rtx_insn *insn)
{
  ...
  return get_attr_z13_unit_fxd(insn) || get_attr_z14_unit_fxd(insn) || get_attr_z15_unit_fxd(insn) || get_attr_z16_unit_fxd(insn);
}
...
s390_sched_score(rtx_insn *insn)
{
 ...
 break a group.  */
    if ((mask & S390_SCHED_ATTR_MASK_CRACKED) != 0 || (mask & S390_SCHED_ATTR_MASK_EXPANDED) != 0)
      score += 5;
 ...
 group.  */
    if ((mask & S390_SCHED_ATTR_MASK_CRACKED) == 0 && (mask & S390_SCHED_ATTR_MASK_EXPANDED) == 0 && (mask & S390_SCHED_ATTR_MASK_GROUPALONE) == 0)
      score += 10;
 ...
 group-of-two insn to avoid shortening another group.  */
    if (sched_state.group_of_two && (mask & S390_SCHED_ATTR_MASK_GROUPOFTWO) != 0)
      score += 15;
    break;
  case 2:
    /* Prefer not cracked insns while trying to put together a
 ...
 CPU.  */
    for (i = 0; i < units; i++, m <<= 1)
      if (m & unit_mask)
        score += (last_scheduled_unit_distance[i][sched_state.side] * MAX_SCHED_MIX_SCORE / MAX_SCHED_MIX_DISTANCE);
    ...
    if (s390_is_longrunning(insn))
    {
      if (s390_is_fxd(insn))
      {
        if (fxd_longrunning[sched_state.side] && fxd_longrunning[other_side] <= fxd_longrunning[sched_state.side])
          score = MAX(0, score - 10);

        else if (fxd_longrunning[other_side] >= fxd_longrunning[sched_state.side])
          score += 10;
      }

      if (s390_is_fpd(insn))
      {
        if (fpd_longrunning[sched_state.side] && fpd_longrunning[other_side] <= fpd_longrunning[sched_state.side])
          score = MAX(0, score - 10);

        else if (fpd_longrunning[other_side] >= fpd_longrunning[sched_state.side])
          score += 10;
      }
    }
  }
...
{
  if (s390_tune == PROCESSOR_2097_Z10 && reload_completed && *nreadyp > 1)
    s390_z10_prevent_earlyload_conflicts(ready, nreadyp);

  if (s390_tune >= PROCESSOR_2827_ZEC12 && reload_completed && *nreadyp > 1)
  {
    int i;
...
s390_sched_variable_issue(FILE *file, int verbose, rtx_insn *insn, int more)
{
  ...
  if (s390_tune >= PROCESSOR_2827_ZEC12 && reload_completed && recog_memoized(insn) >= 0)
  {
    unsigned int mask = s390_get_sched_attrmask(insn);
 ...
 and this insn is the first one of the new group.  */
    ...
    if (s390_tune >= PROCESSOR_2964_Z13)
    {
      ...
      for (i = 0; i < units; i++, m <<= 1)
        ...
        else if (last_scheduled_unit_distance[i][sched_state.side] < MAX_SCHED_MIX_DISTANCE)
          last_scheduled_unit_distance[i][sched_state.side]++;
    }

    if ((mask & S390_SCHED_ATTR_MASK_CRACKED) != 0 || (mask & S390_SCHED_ATTR_MASK_EXPANDED) != 0 || (mask & S390_SCHED_ATTR_MASK_GROUPALONE) != 0 || (mask & S390_SCHED_ATTR_MASK_ENDGROUP) != 0)
    {
      sched_state.group_state = 0;
      ends_group = true;
    }
                ...
                int max_ready ATTRIBUTE_UNUSED)
{
  ...
  rtx_insn *insn = current_sched_info->prev_head
                       ? NEXT_INSN(current_sched_info->prev_head)
                       : NULL;
...
s390_loop_unroll_adjust(unsigned nunroll, struct loop *loop)
{
  ...
  if (INSN_P(insn) && INSN_CODE(insn) != -1)
  {
    ...
    if (s390_tune >= PROCESSOR_2964_Z13 && loop->ninsns <= BLOCK_MEM_OPS_LOOP_INSNS && ((set = single_set(insn)) != NULL_RTX) && ((GET_MODE(SET_DEST(set)) == BLKmode && (GET_MODE(SET_SRC(set)) == BLKmode || SET_SRC(set) == const0_rtx)) || (GET_CODE(SET_SRC(set)) == COMPARE && GET_MODE(XEXP(SET_SRC(set), 0)) == BLKmode && GET_MODE(XEXP(SET_SRC(set), 1)) == BLKmode)))
    {
      free(bbs);
      return 1;
    }
                              ...
                              struct gcc_options *opts_set)
{
  ...
  /* Sanity checks.  */
  if (opts->x_s390_arch == PROCESSOR_NATIVE || opts->x_s390_tune == PROCESSOR_NATIVE)
    gcc_unreachable();
  if (TARGET_64BIT && !TARGET_ZARCH_P(opts->x_target_flags))
    error("64-bit ABI not supported in ESA/390 mode");

  if (opts->x_s390_indirect_branch == indirect_branch_thunk_inline || opts->x_s390_indirect_branch_call == indirect_branch_thunk_inline || opts->x_s390_function_return == indirect_branch_thunk_inline || opts->x_s390_function_return_reg == indirect_branch_thunk_inline || opts->x_s390_function_return_mem == indirect_branch_thunk_inline)
  ...
  if (TARGET_SOFT_FLOAT_P(opts_set->x_target_flags) && TARGET_SOFT_FLOAT_P(opts->x_target_flags))
  {
    if (TARGET_HARD_DFP_P(opts_set->x_target_flags) && TARGET_HARD_DFP_P(opts->x_target_flags))
  ...
  if (TARGET_BACKCHAIN_P(opts->x_target_flags) && TARGET_PACKED_STACK_P(opts->x_target_flags) && TARGET_HARD_FLOAT_P(opts->x_target_flags))
...
s390_option_override(void)
{
  ...
  /* Save the initial options in case the user does function specific
     options.  */
  target_option_default_node = build_target_option_node(&global_options, &global_options_set);
...
#define S390_ATTRIB(S, O, A)  \
  {                           \
    S, sizeof(S) - 1, O, A, 0 \
  }
#define S390_PRAGMA(S, O, A)  \
  {                           \
    S, sizeof(S) - 1, O, A, 1 \
  }
  static const struct
...
#undef S390_PRAGMA
 ...
 the list are the values from the pragma.  */
    num_pragma_values = (!force_pragma && current_target_pragma != NULL)
                            ? list_length(current_target_pragma)
                            : 0;
    for (i = 0; args; args = TREE_CHAIN(args), i++)
    {
      ...
      if (TREE_VALUE(args) && !s390_valid_target_attribute_inner_p(TREE_VALUE(args),
                                                                   opts, new_opts_set,
                                                                   is_pragma))
      {
        ret = false;
  ...
  while (next_optstr && *next_optstr != '\0')
  {
    ...
    {
      opt_len = attrs[i].len;
      if (ch == attrs[i].string[0] && ((attrs[i].has_arg) ? len > opt_len : len == opt_len) && memcmp(p, attrs[i].string, opt_len) == 0)
      {
        opt = attrs[i].opt;
...
tree s390_valid_target_attribute_tree(tree args,
                                      struct gcc_options *opts,
                                      ...
                                      bool force_pragma)
{
  ...
  if (new_opts_set.x_target_flags || new_opts_set.x_s390_arch || new_opts_set.x_s390_tune || new_opts_set.x_s390_stack_guard || new_opts_set.x_s390_stack_size || new_opts_set.x_s390_branch_cost || new_opts_set.x_s390_warn_framesize || new_opts_set.x_s390_warn_dynamicstack_p)
  {
                              ...
                              int ARG_UNUSED(flags))
{
  ...
  if (TREE_VALUE(args) && TREE_CODE(TREE_VALUE(args)) == STRING_CST && TREE_CHAIN(args) == NULL_TREE && strcmp(TREE_STRING_POINTER(TREE_VALUE(args)), "default") == 0)
    return true;
...
s390_can_inline_p(tree caller, tree callee)
{
  ...
  unsigned HOST_WIDE_INT must_match_masks = (MASK_64BIT | MASK_ZARCH | MASK_HARD_DFP | MASK_SOFT_FLOAT | MASK_LONG_DOUBLE_128 | MASK_OPT_VX);

  /* Flags which we in general want to prevent inlining but accept for
     always_inline.  */
  const unsigned HOST_WIDE_INT always_inline_safe_masks = MASK_MVCLE | MASK_BACKCHAIN | MASK_SMALL_EXEC;

  const HOST_WIDE_INT all_masks = (caller_required_masks | must_match_masks | always_inline_safe_masks | MASK_DEBUG_ARG | MASK_PACKED_STACK | MASK_ZVECTOR);
  ...
  bool always_inline = (DECL_DISREGARD_INLINE_LIMITS(callee) && lookup_attribute("always_inline", DECL_ATTRIBUTES(callee)));
  ...
  if (always_inline && ipa_fn_summaries && !ipa_fn_summaries->get(cgraph_node::get(callee))->fp_expressions)
    must_match_masks &= ~(MASK_HARD_DFP | MASK_SOFT_FLOAT);

  if ((caller_opts->x_target_flags & must_match_masks) != (callee_opts->x_target_flags & must_match_masks))
    return false;

  if (~(caller_opts->x_target_flags & caller_required_masks) & (callee_opts->x_target_flags & caller_required_masks))
    return false;
...
void s390_activate_target_options(tree new_tree)
{
...
{
  return (size == 1 || size == 2 || size == 4 || (TARGET_ZARCH && size == 8));
}
...
#define FPC_DXC_SHIFT HOST_WIDE_INT_UC(8)
  ...
  tree old_fpc = build4(TARGET_EXPR, unsigned_type_node, fenv_var, call_efpc,
                        NULL_TREE, NULL_TREE);
  tree new_fpc = build2(BIT_AND_EXPR, unsigned_type_node, fenv_var,
                        build_int_cst(unsigned_type_node,
                                      ~(FPC_DXC_MASK | FPC_FLAGS_MASK | FPC_EXCEPTION_MASK)));
  ...
  *clear = build_call_expr(sfpc, 1, new_fpc);
  ...
  raise_old_except = build2(RSHIFT_EXPR, unsigned_type_node, raise_old_except,
                            build_int_cst(unsigned_type_node,
                                          FPC_FLAGS_SHIFT));
  tree atomic_feraiseexcept = builtin_decl_implicit(BUILT_IN_ATOMIC_FERAISEEXCEPT);
...
s390_vector_alignment(const_tree type)
{
  ...
  if (tree_fits_uhwi_p(size) && tree_to_uhwi(size) < BIGGEST_ALIGNMENT)
...
s390_invalid_binary_op(int op ATTRIBUTE_UNUSED, const_tree type1, const_tree type2)
{
  ...
  /* Mixing signed and unsigned types is forbidden for all
     operators.  */
  if (!bool1_p && !bool2_p && TYPE_UNSIGNED(type1) != TYPE_UNSIGNED(type2))
    return N_("types differ in signedness");

  plusminus_p = (op == PLUS_EXPR || op == MINUS_EXPR);
  muldiv_p = (op == MULT_EXPR || op == RDIV_EXPR || op == TRUNC_DIV_EXPR || op == CEIL_DIV_EXPR || op == FLOOR_DIV_EXPR || op == ROUND_DIV_EXPR);
  compare_p = (op == LT_EXPR || op == LE_EXPR || op == GT_EXPR || op == GE_EXPR || op == EQ_EXPR || op == NE_EXPR);
  ...
  if (bool1_p != bool2_p && plusminus_p && (GET_MODE_CLASS(mode1) == MODE_VECTOR_FLOAT || GET_MODE_CLASS(mode2) == MODE_VECTOR_FLOAT))
...
void s390_rawmemchr(machine_mode elt_mode, rtx dst, rtx src, rtx pat)
{
  machine_mode vec_mode = mode_for_vector(as_a<scalar_int_mode>(elt_mode),
                                          16 / GET_MODE_SIZE(elt_mode))
                              .require();
...
void s390_sched_dependencies_evaluation(rtx_insn *head, rtx_insn *tail)
{
  ...
  {
    rtx set = single_set(insn);
    if (!INSN_P(insn) || !RTX_FRAME_RELATED_P(insn) || set == NULL_RTX || !REG_P(SET_DEST(set)) || !FP_REG_P(SET_SRC(set)))
      continue;
...
bool s390_vectorize_vec_perm_const(machine_mode vmode, machine_mode op_mode,
