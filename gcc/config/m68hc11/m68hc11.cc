...
#define TARGET_FUNCTION_ARG_PADDING m68hc11_function_arg_padding
...
#define TARGET_MODES_TIEABLE_P m68hc11_modes_tieable_p

#undef TARGET_HARD_REGNO_RENAME_OK
#define TARGET_HARD_REGNO_RENAME_OK m68hc11_hard_regno_rename_ok

#undef TARGET_HARD_REGNO_MODE_OK
#define TARGET_HARD_REGNO_MODE_OK m68hc11_hard_regno_mode_ok

#undef TARGET_OVERRIDE_OPTIONS
#define TARGET_OVERRIDE_OPTIONS	m68hc11_override_options

#undef TARGET_CONDITIONAL_REGISTER_USAGE
#define TARGET_CONDITIONAL_REGISTER_USAGE m68hc11_conditional_register_usage

#undef TARGET_HARD_REGNO_NREGS
#define TARGET_HARD_REGNO_NREGS m68hc11_hard_regno_nregs

#undef TARGET_INITIALIZE_TRAMPOLINE
#define TARGET_INITIALIZE_TRAMPOLINE m68hc11_initialize_trampoline

#undef TARGET_NOTICE_UPDATE_CC
#define TARGET_NOTICE_UPDATE_CC m68hc11_notice_update_cc

bool
m68hc11_can_eliminate(int from, int to)
{
...
bool
m68hc11_modes_tieable_p (machine_mode mode1, machine_mode mode2)
{
  return ((mode1) == (mode2)) || ((mode1) != QImode && (mode2) != QImode);
}

unsigned int
m68hc11_hard_regno_nregs (unsigned int regno, machine_mode mode)
{
  return (Q_REGNO_P (regno)) ? (GET_MODE_SIZE (mode)) : ((GET_MODE_SIZE (mode) + HARD_REG_SIZE - 1) / HARD_REG_SIZE);
}
...
/* The _class value for base registers.  */
reg_class m68hc11_base_reg_class = A_REGS;

/* The _class value for index registers.  This is NO_REGS for 68HC11.  */
reg_class m68hc11_index_reg_class = NO_REGS;

reg_class m68hc11_tmp_regs__class = NO_REGS;
  ...
  if (TARGET_M6811)
    {
      ...
      m68hc11_tmp_regs__class = D_REGS;
  ...
  if (TARGET_M6812)
    {
      ...
      m68hc11_tmp_regs__class = TMP_REGS;
...
/* Value is 1 if hard register REGNO can hold a value of machine-mode MODE.
    ...
    - Values whose size is > 32 bit are not stored in real hard
      ...
      enough of them.  */
bool
m68hc11_hard_regno_mode_ok (unsigned int regno, machine_mode mode)
{
  switch (GET_MODE_SIZE (mode))
    ...
    case 1:
      /* We have to accept a QImode in X or Y registers.  Otherwise, the
         ...
         in the insns.  Reload fails if the insn rejects the register _class 'a'
...
reg_class
preferred_reload__class (rtx operand, reg_class _class)
{
  machine_mode mode = GET_MODE (operand);
  if (debug_m6811)
    {
      printf ("Preferred reload: (_class=%s): ", reg_class_names[_class]);
    }

  if (_class == D_OR_A_OR_S_REGS && SP_REG_P (operand))
    return m68hc11_base_reg_class;

  if (_class >= S_REGS && (GET_CODE (operand) == MEM
			  || GET_CODE (operand) == CONST_INT))
    {
      /* S_REGS _class must not be used.  The movhi template does not
  ...
  else if (_class == Y_REGS && GET_CODE (operand) == MEM)
    {
      _class = Y_REGS;
    }
  else if (_class == A_OR_D_REGS && GET_MODE_SIZE (mode) == 4)
    {
      _class = D_OR_X_REGS;
    }
  else if (_class >= S_REGS && S_REG_P (operand))
    {
      switch (_class)
	{
	default:
  ...
  else if (_class >= S_REGS)
    {
      if (debug_m6811)
	{
	  printf ("Class = %s for: ", reg_class_names[_class]);
	  fflush (stdout);
	  debug_rtx (operand);
	}
    }
...
/* Return 1 if the operand is a valid indexed addressing mode.
   For 68hc11:  n,r    with n in [0..255] and r in A_REGS _class
   For 68hc12:  n,r    no constraint on the constant, r in A_REGS _class.  */
int
m68hc11_valid_addressing_p (rtx operand, machine_mode mode, int addr_mode)
{
  rtx base, offset;
...
int
m68hc11_small_indexed_indirect_p (rtx operand, machine_mode mode)
{
  rtx base, offset;
  int addr_mode;
...
int
m68hc11_register_indirect_p (rtx operand, machine_mode mode)
{
  int addr_mode;
...
static int
go_if_legitimate_address_internal (rtx operand, machine_mode mode,
                                   int strict)
{
  int addr_mode;
...
int
m68hc11_go_if_legitimate_address (rtx operand, machine_mode mode,
                                  int strict)
{
  int result;
...
void
m68hc11_emit_libcall (const char *name, rtx_code code,
                      machine_mode dmode, machine_mode smode,
                      int noperands, rtx *operands)
{
  rtx ret, equiv;

  start_sequence ();
  rtx libcall = gen_rtx_SYMBOL_REF (Pmode, name);
  switch (noperands)
    {
    case 2:
  ...
  end_sequence ();
  emit_libcall_block (insn, operands[0], ret, equiv);
}
...
int
memory_reload_operand (rtx operand, machine_mode mode ATTRIBUTE_UNUSED)
{
...
int
m68hc11_symbolic_p (rtx operand, machine_mode mode)
{
  if (GET_CODE (operand) == MEM)
    {
...
int
m68hc11_indirect_p (rtx operand, machine_mode mode)
{
...
int
memory_indexed_operand (rtx operand, machine_mode mode ATTRIBUTE_UNUSED)
{
  if (GET_CODE (operand) != MEM)
    return 0;
...
int
symbolic_memory_operand (rtx op, machine_mode mode)
{
  switch (GET_CODE (op))
    {
    case SYMBOL_REF:
  ...
  if (TARGET_M6811)
    {
      ...
      emit_move_insn (gen_rtx_MEM (HImode, plus_constant (Pmode, tramp, 4)),
                      GEN_INT (0x18df));
      emit_move_insn (gen_rtx_MEM (QImode, plus_constant (Pmode, tramp, 6)),
                      gen_rtx_CONST (QImode,
                                     gen_rtx_SYMBOL_REF (Pmode,
                                                         static_chain_reg)));
      emit_move_insn (gen_rtx_MEM (QImode, plus_constant (Pmode, tramp, 7)),
                      GEN_INT (0x7e));
      emit_move_insn (gen_rtx_MEM (HImode, plus_constant (Pmode, tramp, 8)), fnaddr);
    }
  else
    {
      ...
      emit_move_insn (gen_rtx_MEM (QImode, plus_constant (Pmode, tramp, 6)),
                      GEN_INT (0x06));
      emit_move_insn (gen_rtx_MEM (HImode, plus_constant (Pmode, tramp, 7)), fnaddr);
    }
}
...
const attribute_spec m68hc11_attribute_table[] =
{
  /* { name, min_len, max_len, decl_req, type_req, fn_type_req,
       affects_type_identity, handler, exclude } */
  { "interrupt", 0, 0, false, true,  true, false,  m68hc11_handle_fntype_attribute, NULL },
  { "trap",      0, 0, false, true,  true, false,  m68hc11_handle_fntype_attribute, NULL },
  { "far",       0, 0, false, true,  true, false,  m68hc11_handle_fntype_attribute, NULL },
  { "near",      0, 0, false, true,  true, false,  m68hc11_handle_fntype_attribute, NULL },
  { "page0",     0, 0, false, false, false, false, m68hc11_handle_page0_attribute, NULL },
...
static void
m68hc11_function_arg_advance (cumulative_args_t cum_v,
			     const function_arg_info &arg)
{
  ...
  if (arg.mode != BLKmode)
	...
	{
	  cum->nregs = 2;
	  cum->words = GET_MODE_SIZE (arg.mode);
	}
      else
	{
	  cum->words += GET_MODE_SIZE (arg.mode);
...
static rtx
m68hc11_function_arg (cumulative_args_t cum_v, const function_arg_info &arg)
{
  CUMULATIVE_ARGS *cum = get_cumulative_args (cum_v);
  if (cum->words != 0)
    {
  ...
  if (arg.mode != BLKmode)
    {
      if (GET_MODE_SIZE (arg.mode) == 2 * HARD_REG_SIZE)
	return gen_rtx_REG (arg.mode, HARD_X_REGNUM);

      if (GET_MODE_SIZE (arg.mode) > HARD_REG_SIZE)
	{
	  return NULL_RTX;
	}
      return gen_rtx_REG (arg.mode, HARD_D_REGNUM);
    }
  return NULL_RTX;
}
...
pad_direction
m68hc11_function_arg_padding (machine_mode mode, const_tree type)
{
...
rtx
m68hc11_gen_lowpart (machine_mode mode, rtx x)
{
...
rtx
m68hc11_gen_highpart (machine_mode mode, rtx x)
{
...
print_operand (FILE *file, rtx op, int letter)
{
  ...
  if (GET_CODE (op) == MEM)
	...
	asm_fprintf (file, "%I");
        ...
        need_parenthesize = need_parenthesize (op);
...
m68hc11_expand_compare (enum rtx_code code, rtx op0, rtx op1)
{
  ...
  ret = gen_rtx_fmt_ee (code, VOIDmode, pc_rtx, const0_rtx);
...
m68hc11_split_move (rtx to, rtx from, rtx scratch)
{
  ...
  rtx low_to = m68hc11_gen_lowpart (mode, to);
  rtx high_to = m68hc11_gen_highpart (mode, to);

  rtx low_from = m68hc11_gen_lowpart (mode, from);
  rtx high_from = m68hc11_gen_highpart (mode, from);
  ...
  if (mode == SImode)
		   ...
		   || m68hc11_small_indexed_indirect_p (to, GET_MODE (to)))))
    {
      ...
      // FOR_EACH_SUBRTX (&PATTERN (insn), m68hc11_make_autoinc_notes, insn);
      subrtx_var_iterator::array_type array;
      FOR_EACH_SUBRTX_VAR (iter, array, PATTERN (insn), NONCONST)
        m68hc11_make_autoinc_notes(insn, static_cast<void *>(*iter));
...
static rtx
m68hc11_simplify_logical (machine_mode mode, rtx_code code, rtx operand, rtx *result)
{
  *result = 0;
...
static void
m68hc11_emit_logical (machine_mode mode, rtx_code code, rtx *operands)
{
  rtx result;
  bool need_copy = (rtx_equal_p (operands[0], operands[1])
	       || rtx_equal_p (operands[0], operands[2])) ? 0 : 1;

  operands[1] = m68hc11_simplify_logical (mode, code, operands[1], &result);
  operands[2] = m68hc11_simplify_logical (mode, code, operands[2], &result);
  ...
  if (result && GET_CODE (result) == CONST_INT)
	...
	{
	  insn = emit_insn (gen_rtx_SET (operands[0],
					 gen_rtx_fmt_ee (code, mode,
							 operands[0],
							 operands[2])));
	}
...
static void
m68hc11_split_logical (machine_mode mode, rtx_code code, rtx *operands)
{
  rtx low[4];
  rtx high[4];
...
/* Z register replacement:
   ...
   The Z register is in the A_REGS _class.  In the machine description,
...
static void
m68hc11_find_z_replacement (rtx insn, replace_info *info)
{
  int reg;
  ...
  if (GET_CODE (insn) == INSN && GET_CODE (PATTERN (insn)) == SET)
      ...
      if (Z_REG_P (dst) && (H_REG_P (src) && !SP_REG_P (src)))
	{
	  ...
	  return;
	       ...
	       && ((H_REG_P (dst) && !SP_REG_P (src)) || dst == pc_rtx))
	{
...
int
m68hc11_memory_move_cost (machine_mode mode, reg_class _class,
                          int in ATTRIBUTE_UNUSED)
{
  if (_class <= H_REGS && _class > NO_REGS)
    {
...
/* Cost of moving data from a register of _class 'from' to on in _class 'to'.
   ...
   the constraints.  */
int
m68hc11_register_move_cost (machine_mode mode, reg_class from,
                            reg_class to)
{
  /* All costs are symmetric, so reduce cases by putting the
     lower number _class as the destination.  */
  if (from < to)
    {
      reg_class tmp = to;
      to = from, from = tmp;
...
int
m68hc11_address_cost (rtx x, machine_mode mode ATTRIBUTE_UNUSED,
...
static int
m68hc11_shift_cost (machine_mode mode, rtx x, int shift)
{
  int total;
...
static int
m68hc11_rtx_costs_1 (rtx x, rtx_code code,
                     rtx_code outer_code ATTRIBUTE_UNUSED)
{
  ...
  switch (code)
      ...
      if (GET_CODE (XEXP (x, 1)) == CONST_INT)
	{
      ...
      total = rtx_cost (XEXP (x, 0), mode) + rtx_cost (XEXP (x, 1), mode);
...
bool
m68hc11_rtx_costs (rtx x, machine_mode mode, int outer_code, int opno, int *total, bool speed)
{
  switch (GET_CODE (x))
    {
	...
	*total = 0;
    ...
    case ZERO_EXTEND:
    case IF_THEN_ELSE:
      *total = m68hc11_rtx_costs_1 (x, GET_CODE (x), outer_code);
      return true;
