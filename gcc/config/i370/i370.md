...
;  /* MVS-TODO: "unsigned_jump_follows_p (insn)"*/
;  "*
;{
;  check_label_emit ();
;  if (REG_P (operands[0]))
  ...
  "
{
  if (GET_CODE (operands[1]) == CONST_INT
      && REG_P (operands[0])
      && CONST_CLOBBER_CHECK_OK_FOR_LETTER_P (INTVAL (operands[1]), 'K'))
    {
      emit_insn (gen_rtx_SET (operands[0],
           ...
           && REG_P (operands[0])
	   && CONST_CLOBBER_CHECK_OK_FOR_LETTER_P (INTVAL (operands[2]), 'K'))
    {
      emit_insn (gen_rtx_SET (operands[0],
...
(define_insn ""
  ...
  ; MVS-TODO: "CONST_CLOBBER_CHECK_OK_FOR_LETTER_P (INTVAL (operands[2]), 'K')"
  "*
{
  check_label_emit ();
  mvs_check_page (0, 4, 0);
  ...
  "
{
  ...
  if (GET_CODE (operands[2]) == CONST_INT)
      ...
      else
	{
	  ...
	  emit_insn (gen_rtx_SET (dr_0, operands[1]));
	  emit_insn (gen_rtx_SET (dr_1, const0_rtx));
	  emit_insn (gen_cmpsi (dr_0, operands[2]));
	  emit_jump_insn (gen_bltu (label1));
	  emit_insn (gen_rtx_SET (dr_1, const1_rtx));
	  emit_label (label1);
	}
    }
  else
    {
      ...
      emit_insn (gen_rtx_SET (dr_0, operands[1]));
      emit_insn (gen_rtx_SET (sr, operands[2]));
      emit_insn (gen_rtx_SET (dr_1, const0_rtx));
      ...
      emit_label (label1);
      emit_insn (gen_rtx_SET (dr_1, dr_0));
      emit_jump_insn (gen_jump (label3));
      emit_label (label2);
      emit_insn (gen_rtx_SET (dr_1, const1_rtx));
      emit_label (label3);
    }
  emit_insn (gen_rtx_SET (operands[0], dr_1));
  ...
  "
{
  ...
  if (GET_CODE (operands[2]) == CONST_INT)
      ...
      else
	{
	  ...
	  emit_jump_insn (gen_bltu (label1));
	  emit_insn (gen_rtx_SET (sr, gen_rtx_ABS (SImode, sr)));
	  emit_insn (gen_rtx_SET (dr_0,
  ...
  else
    {
      ...
      emit_insn (gen_rtx_SET (dr,
			  gen_rtx_LSHIFTRT (DImode, dr,
				    gen_rtx_CONST_INT (SImode, 32))));
      emit_insn (gen_rtx_SET (dr, gen_rtx_MOD (DImode, dr, sr)));
      emit_jump_insn (gen_jump (label3));
      emit_label (label1);
      emit_insn (gen_rtx_SET (dr_0, const0_rtx));
      emit_jump_insn (gen_jump (label3));
      emit_label (label2);
      emit_insn (gen_rtx_SET (dr_0,
  ...
  emit_insn (gen_rtx_SET (operands[0], dr_0));
...
(define_insn "bgtu"
  [(set (pc)
	(if_then_else (gtu (reg CC_REGNUM)
			   (const_int 0))
		      (label_ref (match_operand 0 "" ""))
		      (pc)))
;   (clobber (reg:SI 14))
...
(define_insn "bltu"
  [(set (pc)
	(if_then_else (ltu (reg CC_REGNUM)
			   (const_int 0))
		      (label_ref (match_operand 0 "" ""))
		      (pc)))
;   (clobber (reg:SI 14))
...
(define_insn "bgeu"
  [(set (pc)
	(if_then_else (geu (reg CC_REGNUM)
			   (const_int 0))
		      (label_ref (match_operand 0 "" ""))
		      (pc)))
;   (clobber (reg:SI 14))
...
(define_insn "bleu"
  [(set (pc)
	(if_then_else (leu (reg CC_REGNUM)
			   (const_int 0))
		      (label_ref (match_operand 0 "" ""))
		      (pc)))
;   (clobber (reg:SI 14))
